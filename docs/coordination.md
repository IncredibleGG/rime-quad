...
## 候選列的序號、右端保留區與守門的裝置(分支 `dens`,2026-08-13 覆核回修)

上一節那一版被對抗覆核判 `safeToShip: false`,六條全部修掉了。與別端有關的四則:

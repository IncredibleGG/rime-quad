...
而且第 2 條的答案**會隨使用者打到哪裡而翻面**（`MGGAM` 不行、消歧成
`niGAM` 之後可以）。一個「每一刻都正確」的序號，就是一個會在使用者眼前
出現與消失、每次都讓整列重排的序號 —— 那與 §8.6.3.1 守的是同一條紀律。
所以本規範要的是**保守的常數答案**：只要那個 `(佈局, 方案)` 存在任何一個
「按下去不選字」的常見狀態，整格就是**不畫**。

⚠ **視覺上不畫序號與朗讀時不念序號是兩件事。** 無障礙朗讀 **必須** 永遠
帶序號（「第三個，你好」）—— 使用者說「我要第三個」靠的是後者。

實測表的格式與產生方式由渲染端決定，但它 **必須** 是量出來的、不是手寫的
（手寫的表從寫下的那一刻就開始腐爛）。本 repo 的落地是
`core/selection-digit.tsv` ＋ `scripts/verify_selection_digit.sh`
（那支腳本同時是產生器與斷言者）。
...
⛔ **本頁還有畫不出來的候選時，不得提供「下一頁」。** 翻頁鍵的語義是
「本頁我看完了」；本頁沒看完就給翻頁，等於讓使用者**跳過他從未看見的候選**，
而畫面完全正常。這是本規範反覆點名的那一類失敗。
...
畫得出一顆。** 兩條出口同時關不掉的情況**不是**一種要在執行期優雅處理的
頁況，它是**主題的設計錯誤**，而實作 **必須** 在建置期或裝置端把它擋下來
...
43. ⛔ **序號「按下去真的選得到」才畫**（§8.6.1.1，**僅行動端 `candidates.bar`**）。
    ...
    1. 當前層**直接按得到整排 `1`–`9`**（九顆 `send.keysym`，無 modifier）。
       `hint` 不算（角落小字不送出任何東西）；`swipe` 不算（規範第 §9.5 節
       明列 swipe 為 OPTIONAL，行動端可以不實作）；`popup` / `long_press`
    ...
    **檢核方式：斷言綁在「按下去真的選中」，不綁在佈局名字上。**
    ...
    2026-08-13 於 emulator-5558 實測的四格：
    `cn-qwerty-numrow` × `luna_pinyin_tw` / `luna_pinyin` = **畫**；
    `cn-t9-pinyin-numrow` × `t9_pinyin` = **不畫**（按 3 之後輸入框變成 `3⋯`，
    使用者已經打好的組字被毀掉）；`bopomofo-dachen` × `bopomofo_tw` = **不畫**

...
- 四個方案都部署成功、執行期資料齊全
- **使用者詞庫真的會改變輸出**(先證明那個詞打不出來,加詞之後必須打得出來)
...
1. **系統到底認不認得這個輸入來源。** `verify_pkg.sh` 第 3 層用
   `lsregister` 與 `TISCreateInputSourceList` 問過了 —— **在 runner 上查不到**。
   原因是 TIS 掃描 `~/Library/Input Methods` 的時機綁在**有登入的圖形工作階段**,
   而 GitHub runner 是背景工作階段。腳本把這件事印出來但不判失敗。
   → **這一項只有人做得到。** 驗收時請確認「輸入來源」的 + 清單裡看得到
   **RimeQuad 繁體 / RimeQuad 簡體**(不是 `org.rimequad...`),而且有圖示。
2. **設定視窗長什麼樣、每一顆按鈕按下去做了什麼。** 這一輪新增的七頁
...
而「我的詞庫」整頁都是文字框。改成 `LSUIElement` 可以解決,但那會動到

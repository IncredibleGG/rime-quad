...
DLL 在 `ActivateEx` 時問系統「使用者現在用的是哪一份設定檔」,
把 langid 經由 IPC 的 HELLO 帶給服務,服務據此挑方案與字形:

| langid | 方案(依序取第一個裝得到的) | 字形 |
|---|---|---|
...
表在 `common/schema_choice.cc`,判斷是純函式,`run_logic_tests.sh` 有 20 個案例守著。
`verify_installer.sh` 另外對**裝好的**那份 `rime_service.exe` 斷言同一張表
(`--print-choice`),外加一條反向測試:`0x0409`(en-US)必須**沒有意見** ——
少了它,一個「永遠回傳 luna_pinyin」的實作也會讓前三條全過。
...
切過去之後打出來仍然是切之前的字形 —— 而使用者剛剛做的動作看起來完全沒有效果。

⚠ **簡繁不是一個 bool。** 本專案打包的 luna_pinyin 家族**沒有
`simplification` 這個開關**,它用的是一組互斥的 radio:
...
打 `guojia` 出來還是「國家」。而 `rs_set_option` **不會**替你維持 radio 的互斥
(那是 librime 的 switcher 在使用者從選單裡選的時候才做的)——
所以同組另外三個要自己關掉,兩個同時為真的話 t2s 之後會再串一次 t2tw。

### 優先順序(這一段是規範)

**方案:**

1. 設定介面的「預設方案」明著選了某一個 → 對**每一個**語言都成立
2. 這個 langid 上一次實際用的方案(使用者按 `Ctrl+\`` 換過的)
3. 上表的 langid 推導
4. 都不適用 → `schema_list` 第一項

**字形:**

1. 設定介面的「字形」明著選了某一種 → 對每一個語言都成立
2. 上表的 langid 推導
3. 都不適用 → 不送任何 `set_option`(沿用方案自己的 `reset` 值)

第 2 項(記住上次用過的)不是裝飾:少了它,使用者按 `Ctrl+\`` 換了方案,
一切到別的程式就被預設打回去 —— 那顆鍵在他眼裡等於沒有作用。
而「沒有意見」與「選繁體」是兩件不同的事:新裝的機器上把每個人強制設成
傳統漢字,等於替使用者做了他沒有做過的決定。

⚠ **這一段還沒有寫進 `docs/settings-model.md`** —— 併 `main` 的時候那份還不在
(macOS 端還在做)。已照 `docs/coordination.md` §2 回報到 §5,請 macOS 端
在動那份規範時裁決,Windows 端照裁決結果改。
...
把它記進 `schema.last.<langid>`。**全域熱鍵刻意沒有做**:衝突了使用者不會
知道是我們幹的。
...
| 分頁 | 項目 | 立刻生效? | 實際動作 |
|---|---|---|---|
| 方案 | 順序(上移/下移/套用) | 要重新部署 | 改寫 `default.custom.yaml` 的 `schema_list` |
| 方案 | 預設方案 | 立刻 | 寫 `schema.forced`,並對現有每一個 session `rs_select_schema` |
| 文字 | 字形(簡繁) | 立刻 | 寫 `text.variant`,並對每一個 session 送整組 radio |
| 文字 | 標點 | 立刻 | `rs_set_option("ascii_punct")` |
| 文字 | 一次顯示幾個候選 | 要重新部署 | `menu/page_size` 寫進 `default.custom.yaml` |
| 文字 | 候選字大小 | 立刻 | 候選窗的字級倍率 |
| 進階 | 重新整理字詞 | — | `rs_deploy()`,有耗時與結果回饋 |
| 進階 | 開啟使用者資料夾 / 設定檔 | — | `ShellExecute` |

⚠ **「一次顯示幾個候選」改的是引擎的 `menu/page_size`,不是「畫幾個」。**
只截掉畫面上的候選是不行的:數字鍵仍然選得到看不見的那幾個 ——
那是「看得到但摸不到」的鏡像版本,而且更難查。所以它會問使用者
要不要現在重新部署。

⚠ **設定檔裡「沒有的鍵 = 沒設過 = 跟隨預設」。** 這是 Android 端整套設定的
地基,桌面端照抄:設成「跟隨」是**刪掉那個鍵**,不是寫一個哨兵值。
差別在哪天預設值變了 —— 沒表示過意見的人跟著走,明確選過的人不動。
把當下的預設值寫進檔案,這兩種人就再也分不出來,而任何預設值的調整
都會變成「不能動的東西」。

...
- `[2026-08-15] [winfix-usable/Windows → 全體] ℹ️ **記錄要補的是「額度」,不是「行數」。** tsf.log 的按鍵記錄只有 5 行的行程額度、連線失敗只有 6 行,而**唯一沒有額度的那一行**(`ipc_client.cc` 的「連線就緒」)正好是抖動時每 500ms 寫一次的那一行 —— 它會把 400 行的行程額度吃光,把 `ActivateEx` 那幾行真正有價值的訊息擠掉。改法:那一行前 3 次照寫,之後換成每 10 秒最多一行的**累計摘要**(重連幾次、上一條活了多久、斷線期間放行幾鍵、兩趟之間掉了幾鍵);服務端則補 `KEY_MS=`(帶**設定讀取分項**與佇列的三個既有診斷欄位)與連線進出各一行。⚠ 設定讀取那一個分項是刻意的:沒有它,第一名(引擎佇列)與第二名(設定檔 I/O)在總時間上長得一模一樣。⚠ **給四端的一句**:診斷記錄的容量是**共享**的,所以「多加一行」永遠是在跟別的行搶位置;沒有額度的那一行不是最重要的那一行,它只是最吵的那一行。`

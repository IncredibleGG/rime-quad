...
#
# ⚠ **這一支自己不決定中不中止 —— 它回 1,由呼叫端決定。** 檔頭從前寫的是
#   「找不到仍然中止,不跳過」,而兩個呼叫端只有一個是那樣:
#
#     scripts/verify_syllables.sh:503  `rs_find_tesseract || exit 2`
#         OCR **就是**那一關的斷言(消歧欄上讀不讀得到 `ni`)。缺工具 =
#         那一關沒驗到,所以中止 —— 檔頭那句話講的是這一個。
#     scripts/verify_candbar.sh        `rs_find_tesseract 2>/dev/null || true`
#         OCR **不參與斷言**:斷言是數像素,OCR 只在紅了之後把那一塊印出來
#         給看日誌的人。缺工具不影響任何一條判準,中止反而是假紅。
#
#   兩者都對,但檔頭的絕對語氣會讓下一個人把 `verify_candbar.sh` 判成缺陷
#   (或者反過來,把 `|| true` 抄到 `verify_syllables.sh` 上,而那會讓
#   一整關安靜地不驗)。所以規矩改成:
#
#     **呼叫端必須在同一行的註解裡說出「這一關的斷言靠不靠 OCR」。**
#     靠 → `|| exit 2`;不靠 → `|| true`,而且要說出不靠在哪裡。

...
data class KeyboardUiState(
    ...
    /**
     * 「**剛剛那一下**被擋下來了」——擋鍵的那一半回饋。
     *
     * ── 為什麼不能共用 [transientNotice] ────────────────────────────────
     * 因為兩者的優先序剛好相反。[transientNotice] 明文排在 [busyMessage]
     * **之下**(「鍵盤還沒好」比「剛剛切了簡繁」要緊);而這一句非蓋過
     * [busyMessage] 不可 —— 它要回答的正是「我剛才按的那一下怎麼了」,
     * 而底下那句話是使用者按之前就已經在讀的。共用一個欄位的話,兩條規則
     * 必然有一條是錯的。
     *
     * 它畫在鍵區的遮罩上(手指所在的地方),不畫在候選列 —— 引擎沒好的時候
     * 候選列刻意留白,見 `KeyboardView` 那一段。
     */
    val heldKeyNotice: String? = null,

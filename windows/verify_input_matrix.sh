...
run_case() {
  ...
  # ── ⚠ 這個宿主到底有沒有連上服務 ────────────────────────────
  #
  # ⚠⚠ 沒有這一條的話,**18 格裡有 10 格是恆真的**。
  #
  #   `idle_*` 那幾格驗的是「輸入法沒有組字時,鍵要原樣放行」——
  #   而連線整條掛掉時,每一顆鍵也都會原樣放行(那正是 fail-open 的設計)。
  #   於是「輸入法完全不能用」與「輸入法正確地放行」在文件內容上一模一樣,
  #   那 10 格照樣全綠。
  #
  #   2026-08-09 的 main 就是這樣:18 個宿主裡有 8 個在「建立 session」
  #   逾時,而只有 compose_* 那 7 格紅了 —— 剩下的默默通過。
  #   使用者那一側的長相是「選了輸入法之後有時候不能打中文」。
  #
  #   所以現在**每一格**都要斷言「這個宿主真的連上了服務」。
  #   連不上就是連不上,不管那一格的按鍵預期是什麼。
  if grep -aq '連線失敗' "${traf}" 2>/dev/null; then
    note_fail "${name}:這個宿主**沒有連上服務**,所以這一格量到的東西不算數。
     (idle_* 那幾格在連線掛掉時也會通過 —— 每一顆鍵都被放行,
      與『正確地放行』長得一模一樣。這一條就是為了擋那件事。)
$(grep -a '連線失敗' "${traf}" | sed 's/^/       /')"
  fi

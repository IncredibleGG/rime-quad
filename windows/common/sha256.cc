// windows/common/sha256.cc — FIPS 180-4 的 SHA-256。純運算。

#include "sha256.h"

namespace rimewin {
namespace {

inline uint32_t Ror(uint32_t x, int n) { return (x >> n) | (x << (32 - n)); }

const uint32_t kK[64] = {
    0x428a2f98u, 0x71374491u, 0xb5c0fbcfu, 0xe9b5dba5u, 0x3956c25bu,
    0x59f111f1u, 0x923f82a4u, 0xab1c5ed5u, 0xd807aa98u, 0x12835b01u,
    0x243185beu, 0x550c7dc3u, 0x72be5d74u, 0x80deb1feu, 0x9bdc06a7u,
    0xc19bf174u, 0xe49b69c1u, 0xefbe4786u, 0x0fc19dc6u, 0x240ca1ccu,
    0x2de92c6fu, 0x4a7484aau, 0x5cb0a9dcu, 0x76f988dau, 0x983e5152u,
    0xa831c66du, 0xb00327c8u, 0xbf597fc7u, 0xc6e00bf3u, 0xd5a79147u,
    0x06ca6351u, 0x14292967u, 0x27b70a85u, 0x2e1b2138u, 0x4d2c6dfcu,
    0x53380d13u, 0x650a7354u, 0x766a0abbu, 0x81c2c92eu, 0x92722c85u,
    0xa2bfe8a1u, 0xa81a664bu, 0xc24b8b70u, 0xc76c51a3u, 0xd192e819u,
    0xd6990624u, 0xf40e3585u, 0x106aa070u, 0x19a4c116u, 0x1e376c08u,
    0x2748774cu, 0x34b0bcb5u, 0x391c0cb3u, 0x4ed8aa4au, 0x5b9cca4fu,
    0x682e6ff3u, 0x748f82eeu, 0x78a5636fu, 0x84c87814u, 0x8cc70208u,
    0x90befffau, 0xa4506cebu, 0xbef9a3f7u, 0xc67178f2u};

}  // namespace

Sha256::Sha256() : bits_(0), buf_n_(0), done_(false) {
  h_[0] = 0x6a09e667u;
  h_[1] = 0xbb67ae85u;
  h_[2] = 0x3c6ef372u;
  h_[3] = 0xa54ff53au;
  h_[4] = 0x510e527fu;
  h_[5] = 0x9b05688cu;
  h_[6] = 0x1f83d9abu;
  h_[7] = 0x5be0cd19u;
}

void Sha256::Block(const uint8_t* p) {
  uint32_t w[64];
  for (int i = 0; i < 16; ++i) {
    w[i] = (static_cast<uint32_t>(p[i * 4]) << 24) |
           (static_cast<uint32_t>(p[i * 4 + 1]) << 16) |
           (static_cast<uint32_t>(p[i * 4 + 2]) << 8) |
           static_cast<uint32_t>(p[i * 4 + 3]);
  }
  for (int i = 16; i < 64; ++i) {
    const uint32_t s0 = Ror(w[i - 15], 7) ^ Ror(w[i - 15], 18) ^ (w[i - 15] >> 3);
    const uint32_t s1 = Ror(w[i - 2], 17) ^ Ror(w[i - 2], 19) ^ (w[i - 2] >> 10);
    w[i] = w[i - 16] + s0 + w[i - 7] + s1;
  }
  uint32_t a = h_[0], b = h_[1], c = h_[2], d = h_[3];
  uint32_t e = h_[4], f = h_[5], g = h_[6], hh = h_[7];
  for (int i = 0; i < 64; ++i) {
    const uint32_t S1 = Ror(e, 6) ^ Ror(e, 11) ^ Ror(e, 25);
    const uint32_t ch = (e & f) ^ (~e & g);
    const uint32_t t1 = hh + S1 + ch + kK[i] + w[i];
    const uint32_t S0 = Ror(a, 2) ^ Ror(a, 13) ^ Ror(a, 22);
    const uint32_t maj = (a & b) ^ (a & c) ^ (b & c);
    const uint32_t t2 = S0 + maj;
    hh = g;
    g = f;
    f = e;
    e = d + t1;
    d = c;
    c = b;
    b = a;
    a = t1 + t2;
  }
  h_[0] += a;
  h_[1] += b;
  h_[2] += c;
  h_[3] += d;
  h_[4] += e;
  h_[5] += f;
  h_[6] += g;
  h_[7] += hh;
}

void Sha256::Update(const void* data, size_t n) {
  if (done_) return;
  const uint8_t* p = static_cast<const uint8_t*>(data);
  bits_ += static_cast<uint64_t>(n) * 8;
  while (n > 0) {
    const size_t take = (64 - buf_n_ < n) ? (64 - buf_n_) : n;
    for (size_t i = 0; i < take; ++i) buf_[buf_n_ + i] = p[i];
    buf_n_ += take;
    p += take;
    n -= take;
    if (buf_n_ == 64) {
      Block(buf_);
      buf_n_ = 0;
    }
  }
}

std::string Sha256::HexDigest() {
  if (done_) return hex_;
  const uint64_t bits = bits_;
  uint8_t pad = 0x80;
  Update(&pad, 1);
  // ⚠ Update 會把 bits_ 一起加上去,而長度欄位要的是**填充之前**的長度。
  //   所以上面先存了一份 bits。
  pad = 0x00;
  while (buf_n_ != 56) Update(&pad, 1);
  uint8_t len[8];
  for (int i = 0; i < 8; ++i)
    len[i] = static_cast<uint8_t>((bits >> (56 - i * 8)) & 0xff);
  Update(len, 8);

  static const char* kHex = "0123456789abcdef";
  hex_.clear();
  hex_.reserve(64);
  for (int i = 0; i < 8; ++i) {
    for (int b = 3; b >= 0; --b) {
      const uint8_t byte = static_cast<uint8_t>((h_[i] >> (b * 8)) & 0xff);
      hex_.push_back(kHex[byte >> 4]);
      hex_.push_back(kHex[byte & 0x0f]);
    }
  }
  done_ = true;
  return hex_;
}

std::string Sha256::HexOf(const std::string& data) {
  Sha256 s;
  s.Update(data.data(), data.size());
  return s.HexDigest();
}

bool LooksLikeSha256Hex(const std::string& s) {
  if (s.size() != 64) return false;
  for (char c : s) {
    const bool hex = (c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') ||
                     (c >= 'A' && c <= 'F');
    if (!hex) return false;
  }
  return true;
}

bool Sha256HexEqual(const std::string& a, const std::string& b) {
  auto norm = [](const std::string& in) {
    size_t s = 0, e = in.size();
    while (s < e && (in[s] == ' ' || in[s] == '\t' || in[s] == '\r' ||
                     in[s] == '\n'))
      ++s;
    while (e > s && (in[e - 1] == ' ' || in[e - 1] == '\t' ||
                     in[e - 1] == '\r' || in[e - 1] == '\n'))
      --e;
    std::string out = in.substr(s, e - s);
    for (char& c : out)
      if (c >= 'A' && c <= 'Z') c = static_cast<char>(c - 'A' + 'a');
    return out;
  };
  const std::string x = norm(a);
  const std::string y = norm(b);
  // 空字串不算相等 —— 「兩邊都沒有摘要」不是「摘要對得上」。
  if (x.empty() || y.empty()) return false;
  return x == y;
}

}  // namespace rimewin

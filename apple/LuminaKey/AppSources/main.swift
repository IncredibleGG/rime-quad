...
//  `LuminaKey.app/Contents/Resources/LuminaKeySettings.app/Contents/MacOS/`,
...
guard let bundleId = Bundle.main.bundleIdentifier else {
    NSLog("LuminaKey: 沒有 bundle identifier —— 這個執行檔必須從 .app 裡跑")
    exit(1)
}
...
guard let connectionName =
    ...
    NSLog("LuminaKey: Info.plist 缺少 InputMethodConnectionName")
    exit(1)
}
...
guard let liveServer = server else {
    NSLog("LuminaKey: IMKServer 建立失敗(連線名 \(connectionName))")
    exit(1)
}

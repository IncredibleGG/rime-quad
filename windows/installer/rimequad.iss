...
WelcomeLabel2=這將在您的電腦上安裝 [name/ver]。%n%n安裝完成後,請在工作列的輸入法切換清單(Win + 空白鍵)裡選擇它。%n%n建議先關閉其他程式再繼續。
WizardPassword=密碼
...
FinishedLabel=[name] 已安裝在您的電腦上。%n%n用法:按 Win + 空白鍵切換輸入法,選擇「[name]」。%n%n若清單裡沒有出現,請到「設定 → 時間與語言 → 語言與地區」把「中文(繁體,台灣)」加入語言清單,再試一次。%n%n首次使用時輸入法會在背景編譯詞庫,可能需要一到數分鐘;在那之前打出來的是英文,這是正常的。%n%n若打不出中文或看不到設定視窗,請執行「開始」功能表裡的「診斷:輸入法為什麼不能用」,它會把原因寫成一份報告並用記事本打開。

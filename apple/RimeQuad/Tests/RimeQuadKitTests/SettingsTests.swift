...
final class SettingsTests: XCTestCase {
    ...
    func testNilFieldsAreOmitted() {
        let json = String(data: SettingsCodec.encode(RimeQuadSettings()), encoding: .utf8)!
    ...
    func testGarbageIsNotFatal() {
        XCTAssertEqual(SettingsCodec.decode(Data("not json at all".utf8)), RimeQuadSettings())
        XCTAssertEqual(SettingsCodec.decode(Data("[1,2,3]".utf8)), RimeQuadSettings())
        XCTAssertEqual(SettingsCodec.decode(Data()), RimeQuadSettings())
    }

    func testMigrationFillsVersion() {
        let s = SettingsCodec.decode(Data("{\"network_enabled\": true}".utf8))
        XCTAssertEqual(s.version, RimeQuadSettings.currentVersion)
...
final class SettingsCatalogTests: XCTestCase {
    ...
    func testEverySettingsFieldAppearsInTheCatalog() {
        ...
        var missing: [String] = []
        for child in Mirror(reflecting: RimeQuadSettings()).children {
    ...
    func testCatalogFieldsExistOnTheModel() {
        let real = Set(Mirror(reflecting: RimeQuadSettings()).children.compactMap(\.label))
        for item in SettingsCatalog.allItems {

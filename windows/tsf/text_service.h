...
class TextService : public ITfTextInputProcessorEx,
 ...
 private:
  ...
  // 「引擎沒有回答這顆鍵」的額度。**刻意不與 key_trace_budget_ 共用。**
  //
  // ⚠ 這一格就是 §13c 那一趟查不出病因的直接原因:同一段程式碼裡
  //   「引擎不吃這顆字元鍵,由我們補進文件」那一行走的是共用額度,而它
  //   **每天都會走到**(英數模式的每一顆字母)。第一階段打完 nihao1
  //   額度就沒了,於是第二階段真的踩到缺陷的時候,瘦 DLL 的除錯記錄裡
  //   一行都沒有(log4:1170「按鍵記錄額度用完」),而報表的紅字正把人
  //   往那份記錄指。一條只在額度還沒用完時才留得下痕跡的路徑,
  //   等於沒有痕跡。
  //
  // ⚠ 罕見的路徑寫得起自己的額度:它不在每一顆按鍵的路上。
  int not_answered_trace_budget_ = 8;

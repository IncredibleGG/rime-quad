...
# ⛔ 這一支是**唯一**沒有寫 device stamp 的裝置端腳本 —— 於是它的 artifact
#   事後答不出「跑在哪一台、量的是哪一份輸入法」。這台機器上三台 AVD 全是
#   1080×2400 @420dpi,截圖本身分不出是哪一台。
# ⚠ 寫在安裝與 `ime set` **之後**:要記的是裝置上真的裝著、而且真的被設成
#   預設的那一份(`pm path` → 裝置端 sha256),不是「我打算裝上去的那一份」。
#   本支不自己裝輸入法(它驗的是既有的那一份),所以第 4 個參數留空。
rs_write_device_stamp "$ADB" "$SERIAL" "$OUT_DIR/device.txt" "" "$IME_PKG"

...
2. **重新開機。**
   ⚠ 這一步是**舊版那支解除安裝程式**的要求,不是新版的。舊版原地覆蓋
   / 原地刪除那顆瘦 DLL,刪不掉時會排進「開機時刪除」的佇列並要求重啟,
   而在那之前它也裝不回來。**登出再登入不夠** —— 那份佇列只有開機時才處理。
   新版已經不這樣了(改名挪開,見下面「安裝與更新不得要求重新啟動」),
   但你要先跑完的是舊版留下的那一支,所以這一次照它的規矩走。
3. **再安裝新版。**
...
決策紀錄:[`docs/decisions/no-restart.md`](../docs/decisions/no-restart.md)。
使用者的原話是「不要每次安裝都重啟啊。這個是大工程,任何端都不能這樣」,
而理由不是體驗:

> **一個每次更新都要重開機的輸入法,使用者就不會更新它。**
> 然後安全性修正、方案沙盒的加強、崩潰修復,全都到不了他手上 ——
> 而這個專案的定位是經得起審計,那句話建立在
> 「使用者手上跑的是我們現在的程式碼」之上。
>
> **更新阻力是安全性問題,不是體驗問題。**
...
`rime_tsf.dll` 會被載入到**每一個**接受文字輸入的進程裡(檔案總管、
瀏覽器、Office)。被載入的檔案**覆蓋不了也刪不掉** —— 但**改得了名字**:
Windows 的載入器開映像檔時給的是 `FILE_SHARE_READ | FILE_SHARE_DELETE`,
而 DELETE 那一格同時涵蓋重新命名。

所以安裝程式的 `PrepareToInstall` 先做一件事:

```
{app}\rime_tsf.dll  →  {app}\rime_tsf.dll.old-<時間戳>-<序號>
```

於是 Inno 走到 `[Files]` 那一行時目的地根本不存在,直接寫一個新檔:

| | |
|---|---|
| 已經開著的程式 | 繼續用手上那份舊映像(它跟著改名後的檔案走)—— 無害 |
| **新開的程式** | 從 `InprocServer32` 指的那個固定路徑拿到**新檔** |
| 服務進程 | 安裝前就被停掉了,直接換掉,本來就不需要重啟 |
| 排進開機佇列的東西 | **一個都沒有** → 安裝程式不會問要不要重新啟動 |

舊檔什麼時候消失:**下一次安裝**、或**服務進程下一次啟動**時各掃一遍
(`rime_ime_setup.exe sweep-stale-dlls` / `service/main.cc`)。
⚠ **掃不掉不是錯誤,一次都不是** —— 掃不掉只代表還有進程握著它,
那顆檔案佔幾 MB、不影響任何功能。為了它要求使用者重開機,
正是這一輪要消滅的東西。所以那個動詞**永遠以 0 結束**。

#### 解除安裝:告知,不強迫

反註冊的當下輸入法就已經停止作用了(CLSID 與 `CTF\TIP` 整棵刪掉,
沒有任何新進程會再載入它)。剩下的只是磁碟上那顆檔案,
而使用者重不重啟**對他沒有任何差別**。

所以解除安裝走同一條路:先改名挪開,再刪那個改名後的檔。

| 情境 | 結果 |
|---|---|
| 沒有程式握著它 | 直接刪掉,不留痕跡,Inno 什麼都不會問 |
| 有程式握著它(§11 用 `--hold-dll` 重現) | 刪不掉,由 `sweep-stale-dlls --schedule-if-locked` 排進開機清除。那是**我們**排的,Inno 不知道,所以**它不會要求重新啟動** |
| 不重新啟動就重裝 | **裝得回來**(§11 斷言結束碼必須是 0)。佇列裡已經沒有那個固定路徑,`PreviousInstallNotCompleted` 沒有東西可以擋 |
| 那份佇列什麼時候會被處理 | **只有開機時**(Session Manager)。登出再登入不會碰它 —— 文案仍然明講這一點 |

⚠ 最後一列是**行為改變**:舊的作法讓 Inno 把 `rime_tsf.dll` 本身排進佇列,
於是重裝會被擋下來(當時實測結束碼 8),舊文案因此寫著「在重新啟動之前
您裝不回來」。**那句話現在不成立了,所以它被刪掉了**,而 §11 把
「裝得回來」從量測升級成斷言 —— 文案宣稱做得到而實際做不到,
是這個專案吃過虧的那一種。

#### 這一整條怎麼證明它不是假的

`verify_installer.sh` §13 在**有進程真的握著舊 DLL** 的狀態下裝新版,
然後斷言四件事:

1. 安裝記錄裡有「已改名挪開」(機制**真的跑了**,不是碰巧沒撞到問題)
2. `rime_tsf.dll` 的 **NTFS 檔案 id 變了** —— 那個路徑上躺的是另一顆檔案,
   也就是新開的進程拿得到新版,不必等重新開機
3. **舊的 DLL 映像 + 新的服務**照樣打得出「你好」(§13c,兩階段宿主)
4. `PendingFileRenameOperations` 裡**沒有**目的地是 `rime_tsf.dll` 的項目

⚠ 而 §14 是它唯一的擔保:用 `/LEGACYINPLACE` 跑一次**跳過改名挪開**的安裝,
並要求第 4 條真的紅。沒有 §14 的話,第 4 條有可能只是因為 runner 上沒有人
握著 DLL 而通過 —— 也就是一個永遠綠、永遠沒在測的關卡。
這個專案已經抓過四次那種東西。

#### 仍然驗不到的

* **兩個版本的位元組真的不同時會怎樣。** §13 的「新版」是同一次建置再裝
  一次,所以它驗到的是換檔機制與升級後的相容性,**不是**跨版本的線路相容。
  那一格靠 `protocol.h` 的版本協商與 `tests/test_proto_compat.cc`,
  而 `rime_shell` 的 ABI 那一格**沒有降級的路** —— 不合就是不合。
* **真的檔案總管 / 瀏覽器 / Office**。§13 用的是我們自己寫的假宿主。
...
| langid | | 描述字串 | 註冊(HKLM) | 啟用(HKCU) |
|---|---|---|---|---|
| `0x0404` | zh-Hant-TW | LuminaKey 輸入法 | 一律 | 只有選中時 |
| `0x0804` | zh-Hans-CN | LuminaKey 输入法 | 一律 | 只有選中時 |
| `0x0C04` | zh-Hant-HK | LuminaKey 輸入法 | 一律 | 只有選中時 |

⚠ **這一欄與那一欄不是同一件事,而它們曾經被當成同一件事。**

第一版只註冊了 `0x0404`,結果**系統語言是簡體中文的使用者在自己的語言底下
找不到這個輸入法** —— 它掛在「繁体中文(中国台湾)」那一欄。使用者實際回報過。
修法是註冊三份,而且**那個修法是對的,不可以退回去**。

但接著使用者回報了第二件事:他的 Win + 空白鍵清單上長這樣 ——
...
- `RegisterProfile`(HKLM)讓這個輸入法在那個語言底下**可以被加入**。
  它自己不會出現在任何人的清單上。三份註冊在清單上是**零格**。
- 清單上出現幾格,由**啟用**決定。而舊版有兩條路同時在啟用:
  1. `RegisterProfile` 的 `bEnabledByDefault = TRUE` —— 那是**全機、對所有
     使用者**的預設啟用(對照 `EnableLanguageProfileByDefault` 的文件寫的是
     for all users,`EnableLanguageProfile` 才是 for the current user);
  2. 安裝程式的 `enable-user` 對三份各呼叫一次 `EnableLanguageProfile`,
     而且刻意「不因為其中一份失敗就放棄其餘的」—— 那個設計假設
     「哪幾份會成功取決於使用者的系統」,但**三份都會成功**。

截圖上那兩欄繁體中文底下**只掛著 LuminaKey**,就是這件事的指紋:
他若真的自己裝過那兩個語言,微软注音之類的東西會跟著出現在同一欄。
是我們把那兩個語言加進他的清單裡的。

**現在的契約:註冊三份,啟用正好一份。**

選哪一份由 `windows/common/profile_choice.cc` 決定,依序:

1. `--lang`(使用者明確指定);
2. **他的語言清單,照他自己的順序**(`HKCU\Control Panel\International\User
   Profile` 的 REG_MULTI_SZ `Languages`)—— 這一層回答「他在哪一欄底下
   找得到我們」,也是最早那個 bug 真正該有的修法;
3. 系統回報「已經有輸入法」的語言(`GetLanguageList`);
4. 系統顯示語言;
5. 全落空 → `zh-Hans-CN`。⚠ 這一格會讓 Windows **替他新增一個語言**,
   不得不然:不加的話裝完之後清單上沒有我們。往簡體那邊倒的理由寫在
   `profile_choice.h`(落到這一格的人是我們一無所知的人,而最早的 bug
   正是簡體使用者找不到)。

⚠ **判斷是純函式,不 include windows.h。** CI 的 Windows runner 只有一種
語言情境(英文、沒有任何中文輸入語言),所以在那裡只走得到第 5 格。
...
沒有說「設定 UI」涵蓋哪幾個介面(Win + 空白鍵的切換器與「新增鍵盤」是
不同的介面),`TF_INPUTPROCESSORPROFILE` 也讀不回這個旗標,而且找不到
任何一個公開的輸入法在用它(Weasel、Mozc、chewing 都沒有)。拿一個
沒有人走過、而且**讀不回來所以驗證不了**的旗標,去修一個我們已經知道
怎麼修的問題,等於把「清單上有幾格」交給猜測。啟用那一條量得到。

**升級的清理是同一條路。** `enable-user` 先開要留的那一份、再關其餘的
(順序不可反:先全關的話中間有一瞬間使用者一份都沒有),冪等。
舊版留下的三份會在升級時被收回去。CI 用 `enable-user-legacy-all`
真的重現舊版的行為,再跑一次 `enable-user` 並斷言回到一份 ——
直接連跑兩次 `enable-user` 驗不到這件事,第二次面對的已經是乾淨狀態,
而**前提不成立的斷言必定通過**。

⚠ **代價:同一台機器上的第二個使用者不會自動拿到這個輸入法**,
因為沒有人替他跑過 `enable-user`。自動給他的話,給的會是三份
(我們無從得知他的語言清單),而那正是要修的問題。

⚠ **語言標籤與實際打出簡繁是兩件事。** 清單上顯示成哪一種中文由 langid
決定;實際上屏的是簡體還是繁體字由 RIME 方案(`luna_pinyin` vs
`luna_pinyin_tw`)與簡繁開關決定,與 langid 無關。

### ⚠ 收斂成一份之後,簡繁必須在我們自己的 UI 裡切得到

這是上面那個修正**拿走**的東西:以前使用者可以用 Win + 空白鍵在
「简体中文」與「繁体中文」兩份設定檔之間切,而那實際上就是在切字集
(langid → `CharSetOfLangId` → `DecideVariant`)。只剩一份之後那條路沒了。

所以 `text.variant` 從「一個補充設定」變成**主要的控制項**,而它在三個
地方都要能改:

| 入口 | 在哪裡 | 狀態 |
|---|---|---|
| 設定的「文字」分頁 | `settings_window.cc` 的 `IDC_VARIANT_COMBO` | 本來就有 |
| 系統匣圖示的右鍵選單 | 三個 radio(跟隨 / 繁體字 / 簡體字),`MF_CHECKED` 標出目前是哪一個 | **這一輪加的** |
| 語言列上的按鈕 | 仍然只是「開啟設定」,**沒有**做成選單 | 見下 |

三條路共用 `CommitVariantPref` 一支 —— 各寫一份的漂移症狀是
「從系統匣切有效、從設定切無效」,而使用者只會說「這個輸入法有時候會亂跳字」。

⚠ **語言列按鈕沒有做成簡繁選單,這是刻意的,理由寫在這裡。**
那顆按鈕住在瘦 DLL 裡(`ITfLangBarItem` 是 TSF 向文字服務要的介面,
只能在那裡),而那支 DLL 被載入到每一個接受文字輸入的進程 ——
瀏覽器、Office、提權視窗。要在上面做選單,得加 `ITfMenu`、加一個新的
單向 IPC op(線路版本要升到 3)、還要讓 DLL 知道目前是簡是繁。
而**那顆按鈕現在是唯一一個已經在運作的入口**,把它從「按一下開設定」
改成「選單」,是拿一個可用的東西去換一個這一輪驗不到的東西。
`Snapshot` 的 `kStSimplified` 已經在線路上了,所以日後要做時
材料是齊的 —— 但那要與「有人真的在真機器上按過那顆按鈕」同一輪做。

第一格下拉的字也跟著改了:「跟著我選的輸入法語言」→「跟著輸入法所在的語言」。
前者的前提是清單上有好幾份可以選,而那個前提已經不成立了。
...
| **解除安裝之後不重開機就裝得回來** | §11(文案宣稱做得到,這一條驗它) |
| **升級時安裝程式真的把舊 DLL 改名挪開了** | §13b(機制有跑,不是碰巧沒撞到) |
| **升級之後 `rime_tsf.dll` 是另一顆檔案(NTFS 檔案 id 變了)** | §13b(新開的進程拿得到新版) |
| **舊的 DLL 映像 + 新的服務照樣打得出「你好」** | §13c(使用者升級之後的真實狀態) |
| **升級沒有把任何東西排進開機佇列** | §13b(也就不會問要不要重新啟動) |
...
- 解除安裝**之後** `check` 必須再度紅
- **§14:帶 `/LEGACYINPLACE` 跳過改名挪開,在有人握著 DLL 的狀態下安裝,
  §13 的「沒有排進開機佇列」必須真的紅。** 沒有這一條的話,那一關有可能
  只是因為 runner 上沒有人握著 DLL 而通過 —— 一個永遠綠、永遠沒在測的關卡
...
#### ⚠ 設定介面(2026-08-09 這一輪)的「只有人看得到」清單

`windows/check_ui_spec.sh` 的 22 組檢核與 `run_logic_tests.sh` 的 213 個案例
**沒有一條需要真的 Windows** —— 那是刻意的(版面、色票、狀態列位置還原、
候選窗排版全部抽成純函式)。**但也因此,它們一條都證明不了下面這些:**

| # | 只有人看得到的 | 為什麼機器看不到 |
|---|---|---|
| U1 | **側欄式版面畫出來好不好看**、四頁的疏密是不是一致 | 版面只驗得到「數字對不對」(W18/W19),驗不到「看起來對不對」 |
| U2 | **深色模式實際長什麼樣**,尤其是**核取方塊與單選鈕的方塊字形**、以及 **ListView 的捲軸** —— 它們在深色下仍然是系統的淺色繪製,這是**刻意接受**的瑕疵(§12.7.3),不是 bug | 需要真的 uxtheme 在畫 |
| U3 | **標題列的深色**有沒有生效。`DWMWA_USE_IMMERSIVE_DARK_MODE` 先試 20 再試 19,兩個都失敗就放著(淺色標題列 + 深色內容) | 兩個屬性值在哪一版 Windows 上生效,只有真機答得出來 |
| U4 | **高對比模式**下整份色票換成 `GetSysColor()` 之後畫面能不能看 | 要真的開高對比 |
| U5 | **高 DPI**(125% / 150% / 175%)下的實際字高與版面。⚠ `MulDivRound` 的**數學**有單元測試,但「字看起來對不對」沒有 | 需要一台高 DPI 螢幕 |
| U6 | **跨螢幕拖動**時 `WM_DPICHANGED` 的重建有沒有真的接上(字型重建、版面重算、狀態列重新定位) | 需要兩顆不同 DPI 的螢幕 |
| U7 | **字型解析的結果**。⚠ 特別是 `Segoe UI Variable Text` 這個確切字串在 Win11 上存不存在 —— 錯了會安靜地退到 `Segoe UI`(退化路徑是對的,所以**錯了也不會壞**,只是拿不到想要的字體) | `EnumFontFamiliesExW` 要真的機器上的字體清單 |
| U8 | **懸浮狀態列**:那一橫的實際尺寸與位置好不好用、拖動手感、四格點不點得準、方案彈出清單會不會被邊緣切到 | §12.13 第 9 條自承「每一個尺寸都是推的,沒有畫出來看過」 |
| U9 | ⚠ **中英切換是不是真的切了。** 這是這一輪最重要的一條人工驗證:在記事本裡打一半中文,點那一橫的第一格,確認**接下來打的是英文**,而且**不影響已經打出來的字、不讓輸入框失去插入點**;然後開一個新程式,確認它**也**是英文 | 需要真的宿主與真的 librime session |
| U10 | **系統匣圖示**長什麼樣(目前用系統的 `IDI_APPLICATION`,醜但看得到) | 需要真的工作列 |
| U11 | **每一顆控制項是不是真的做了它宣稱的事**(§2-D2)。這個專案抓過**六個**「畫面正常、自動化全過」的鍵,而第六個(`IDC_FOLLOW_MODE`)正是這一輪修掉的 | 只有逐項點過才算 |
| U12 | **文案讀起來對不對**。W9 只驗得到「沒有禁用字」,驗不到「這句白話說得對不對」;三個語系的翻譯品質同理 | §6.7 第二層明寫「這一條靠人看」 |
| U13 | **停用控制項旁邊那句話說得對不對**(W23 只驗得到它**存在**) | §12.12 自己標的「機器只驗得到一半」 |
| U14 | **候選窗的 `…`**:`shrink` 溢出時尾端補的省略號畫出來對不對(排版數字有單元測試,像素沒有) | 需要真的 GDI |
| U16 | ⚠ **「正在準備」那一句在真機上到底出現多久。** 2026-08-10 這一輪加的。第一次安裝完立刻看右下角那一橫:它應該寫「正在準備,馬上就好」而**不是**紅色的「輸入法沒有在跑」;等字詞整理完(7~12 秒),那一橫要**自己**變回四格,**不必**先去某個輸入框打一個字。同一段時間裡設定視窗側欄底部那一行也要跟著動 | 三種處境 → 三句話的**判斷**有單元測試(test_service_state.cc),呼叫位置與資料流有 W27,但「那 7~12 秒裡畫面上真的是那一句」需要一次真的首次部署 |
| U15 | ⚠ **設定視窗真的捲得動嗎。** 2026-08-10 這一輪加的。四條路各點一次:滾輪、拖捲軸、Tab 走到視窗外的控制項(畫面要跟著捲,焦點環要看得見)、以及**最小尺寸(660×460)下把「外觀」頁捲到底,確認深淺色三態、狀態列開關、下面那句說明全部碰得到** | 捲動的**數字**有單元測試(可視高度 + 捲動上限 ≥ 每一顆的 bottom),但「滾輪事件有沒有真的到得了父視窗」只有真機答得出來 |
| U16 | **捲到一半的控制項有沒有畫到底部那一列上。** 內容是用 `SetWindowRgn` 裁在底部 hairline 上的 —— ListView / 唯讀 EDIT / 自繪的危險鍵三種都要看過 | `SetWindowRgn` 對每一種控制項的實際效果只有 GDI 知道 |
| U17 | **捲動時會不會閃。** 加了 `WS_CLIPCHILDREN`,但 `LayoutUi` 仍然每次 `InvalidateRect(nullptr)` | 重畫的觀感沒有任何自動化量得到 |
| U18 | ⚠ **懸浮狀態列從「未就緒」變成「就緒」的那一瞬間。** 服務剛起來時那一橫是一格(約 114 DIP),就緒後變四格(約 220 DIP)。以前只往右長,右端會被推出螢幕;現在寬度一變就重走 `PlaceStatusBar`。要看的是:**變寬之後「設定」那一格還在螢幕裡、右緣仍然貼著邊 12 DIP、而且沒有先往右跳一下再彈回來** | 那支定位函式是純的、有單元測試;但「Relayout 之後有沒有真的呼叫它」只有真機看得到 |
| U19 | ⚠ **点 简/繁 那一格,字有沒有立刻變。** 以前要等使用者真的打一個字才會變,而且在那之前再點一次送的是同一個值。連點兩次:第一次 繁→简,第二次要 简→繁 | 指示器的值來自引擎快照,而快照要真的按鍵才會產生 |

⚠ **U15、U18、U19 是 2026-08-10 這一輪最該先點的三條**(U15 更是:
在它之前,「外觀」頁有 384 DIP 排在視窗外面,而深淺色三態是整個 UI 上
唯一的入口)。

⚠ **U9 與 U11 是 2026-08-09 那一輪最該先點的兩條。** 其餘的壞掉只是難看;
這兩條壞掉的話,使用者會遇到的是「這顆按鈕沒有反應」——
而那正是這個專案抓過六次的那一族。
...
4. **提權的視窗上能不能打字**(`GUID_TFCAT_TIPCAP_SECUREMODE`)。
   相關的規則這一輪改對了:DLL 拒絕啟動服務的條件不再是「這個宿主提權了」,
   而是「**這個使用者在這個工作階段裡另有一份非提權的身分會被弄壞**」
   (見 `common/elevation_policy.h`)。內建 Administrator 帳號與關掉 UAC 的
   機器上整個工作階段都是提權的,舊規則等於讓那些人**永遠**用不了輸入法,
   而且沒有任何錯誤訊息 —— 2026-08 的實測回報就是這一種。
   ⚠ 仍然沒被人驗過的是:**分裂權杖**的機器上,提權視窗裡那顆語言列按鈕
   顯示「未啟動」長什麼樣、使用者看不看得懂。
5. **首次部署那幾分鐘的行為。** 服務在部署完成前對每一顆按鍵立刻回「沒處理」,
   使用者應該看到的是「輸入法還沒好,打出來是英文」而不是卡住。
6. ~~**「每一顆鍵是不是都真的做了它宣稱的事」。**~~
   → **這一項這一輪搬走了一大半**,而它是被使用者先抓到的:
   「居然無法刪除字,可以打字,不能刪除」。成因是 `OnTestKeyDown` 對每一顆
   映得出 keysym 的鍵都宣告吃掉,而引擎在沒有組字時不處理退格 ——
   那顆鍵掉進宿主與我們中間的黑洞(見 `common/key_eat_policy.h`)。
   現在有兩層守門:`tests/test_key_eat_policy.cc` 的逐鍵真值表,
   與 `verify_input_matrix.sh` 的按鍵矩陣(每顆鍵在「組字中」與「沒有組字」
   兩種狀態下各驗一次,**斷言的是文件內容**)。
   ⚠ 仍然沒被驗到的:帶修飾鍵的組合(Ctrl+C / Alt+F4)只有真值表驗得到,
   假宿主的送鍵路徑不帶修飾鍵狀態;以及真的宿主(記事本、瀏覽器)在
   「輸入法說不吃」之後**到底做不做預設處理** —— 我們的假宿主一定做。
7. **`ToUnicodeEx` 的死鍵狀態沒有被我們吃掉。** 程式碼帶了「不改動鍵盤狀態」
   的旗標,但那件事只有真的按一次法文的 `^` 再按 `e` 才驗得到。
8. **`ActivateEx` 背景啟動服務那條路,在真的宿主裡的時序。**
   → `verify_installer.sh` §5c 這一輪把它從「紙上的」搬下來了:
   空的使用者資料目錄、沒有人先跑服務,只切一次輸入法,然後斷言
   `rime_service.exe` 真的出現、詞庫編譯完、第一次打字打得出「你好」。
   ⚠ 仍然沒有人看過的是**使用者眼睛看到的那件事**:切過去之後幾秒內
   系統匣圖示會不會出現。CI 量的是進程存在,不是像素。
...
13. **語言列上那顆按鈕長什麼樣、在真宿主上出不出現。**
    ⚠ 這一項這一輪**縮小了但沒有消失**:`verify_tsf.sh` 現在讀瘦 DLL 的
    落地記錄,斷言「語言列按鈕:已加入」那一行真的出現 —— 也就是
    按鈕確實被建出來、而且被宿主的語言列項目管理員收下了。
    仍然沒有被驗證的是**它畫出來長什麼樣**、`TF_LBI_STYLE_SHOWNINTRAY`
    在真的工作列輸入指示器上有沒有效、以及 `GetIcon` 回 `E_FAIL`
    是不是真的會退回顯示文字。那三件事要有人看。
14. **按下那顆按鈕之後設定視窗會不會出現。**
    ⚠ 這一項這一輪**走掉了兩條路**:`verify_installer.sh` §12b/§12c 真的
    執行了 CreateProcess(服務沒在跑 → 自己起來開窗)與具名事件
    (服務在跑 → 第二支把訊息傳過去)兩條,並用 `FindWindowW` 問類別名
    確認視窗真的開出來了。
    **第一條(IPC 的 `kOpenSettings`)仍然沒有被走過**,而 UWP / 市集 App
    的宿主只有那一條走得通 —— 那正是最可疑的一格。
...
19. ~~**`--settings` 在已經有服務在跑時真的把訊息傳過去了。**~~
    → 這一輪由 `verify_installer.sh` §12c 驗到了(具名事件那條路)。
...
23. **開始功能表那兩個捷徑按下去會怎樣。**
    ⚠ 存在、目標與參數這一輪由 `verify_installer.sh` §12a 斷言了
    (讀 .lnk 的 TargetPath / Arguments 逐項比對)—— 參數打錯的症狀
    正是「點了沒反應」,而這個專案抓過四次那種鍵。
    但**按下去之後的體驗**沒有人看過:「診斷」那一個會開一個主控台視窗、

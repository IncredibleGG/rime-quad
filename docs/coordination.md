...
- `[2026-08-12] [Windows] **「整理字詞期間打字」的機制上一輪寫錯了:行為是對的,原因不是那個。凡是「核心一條序列化佇列 + 宿主那側每顆鍵有逾時」的端都要看一遍。**(#90 覆核)`
  ...
  - `而 DLL 那一側每一顆按鍵的預算當時是 **50 毫秒**(`windows/tsf/ipc_client.cc` 的 `kKeyTimeoutMs`)。所以按下「重新整理字詞」之後的第一顆鍵是:吃滿 50 ms → `Fail(kTimeout)` → `Close()`、`session_ = 0` —— **整條連線被丟掉**。`
  - `⚠ **那兩個數字在 2026-08-15 那一輪改掉了**,而且搬了家:現在是 `windows/common/key_deadline.h` 的 `kKeyTimeoutMs = 150`(DLL 一趟)與 `kKeyDeadlineMs = 100`(服務端),由 static_assert 把「服務端必須先放棄」的關係綁住。凡是引用「50 毫秒」的地方都已經過期。`
...
## 「查不到」不等於「是別人的」(分支 `winfix-usable`,2026-08-15,#111)

上一輪把懸浮狀態欄從「有沒有人連著我們」改成「使用者此刻焦點那條執行緒上啟用中的
是不是我們」,方向是對的,而它多了一個沒說出口的前提:**每一個前景執行緒都會有一條
在場連線**。那個前提不成立,於是判準把「我查不到」讀成「那是別人的」,那一橫就在
使用者眼前消失。這一節記的是**形狀**,因為 macOS 的狀態指示器與 Android 的
「現在是哪一個輸入法」都問同一個問題。

- `[2026-08-15] [winfix-usable/Windows → 全體] ⛔ **判準只有兩個出口(是我們 / 不是我們)時,「我不知道」會被靜靜地塞進其中一個 —— 而它總是被塞進破壞性的那一個。** `windows/common/bar_owner.cc:98` 的 `if (!best) return out;`:掃過每一條連線都對不上就回 in_use=false,3000 毫秒後隱藏。落在這一行的**不只是**「使用者換了輸入法」,還有截圖工具的框選疊層、工作列、桌面 —— 它們從來不載入我們的 TIP,一條連線都不會有。修法是把它拆成三態(`kOurs` / `kTheirs` / `kHold`),而 **`kTheirs` 需要正面證據**:宿主自己說出「這條執行緒上啟用中的不再是我們」。⚠ **給四端的一句**:任何「跟著焦點走」的指示器,列舉出口時要問「查不到的時候落在哪一格」,而那一格必須是**維持**,不是兩個確定答案之一。三態要是**具名的值**,不是兩個布林拼出來的 —— 拼出來的寫不進記錄,而這一輪之所以要靠推理定位,就是因為服務端從來沒寫下「我這一刻判成什麼」。`
- `[2026-08-15] [winfix-usable/Windows → 全體] ⛔ **升級之後,使用者桌面上一半的程式還載著舊的那份行程內元件,而且到重開機為止都不會換。** 使用者的 tsf.log 分界乾淨到零例外:08-09 / 08-14 載入 DLL 的行程(Explorer.EXE、Snipaste、rustdesk…)一行新版才有的紀錄都沒有,08-15 重裝後才開的全都有。Explorer.exe(工作列、檔案總管)幾乎永遠不會重開。⚠ **給四端的一句**:凡是「注入到別人行程裡」的東西(Windows TSF DLL、macOS 的 input method bundle),「已安裝的版本」與「正在跑的版本」是兩個不同的量,而**混用它們的後果會集中在最常駐的那幾個宿主上**。這一輪的做法是讓服務端在偵測到舊版宿主連上來時往記錄寫一行(帶行程名與 pid),明講「這個程式在你升級前就開著,關掉再開」——**寫記錄,不做彈窗**:那是一個使用者當下沒辦法處理的東西。`
- `[2026-08-15] [winfix-usable/Windows → 全體] ℹ️ **證據的壽命不可以綁在傳輸它的那條連線上。** 「使用者切走輸入法」以前唯一的表達方式是**把連線關掉**,而「關掉」與「宿主死了 / 宿主凍結 / 宿主根本沒載入我們」在服務端是同一個事件。改成一則明確的訊息(線路 v4 的 `kProfileState`)之後,那筆證據存在服務端、鍵是 tid、活得比連線久 —— 因為 TSF 常常在讓位之後緊接著把那條執行緒上的 TIP 也卸掉。⚠ 這個設計最容易被下一輪「順手修好」的一步就是在連線關閉時把它清掉,所以那一格配了一條**負面守門**(`audit_single_source.sh`:OnClientDetached 內不得出現 `yields_`)與一個反向注入。`
- `[2026-08-15] [winfix-usable/Windows → 全體] ℹ️ **線路版本往上跳不會擋掉舊用戶端 —— 但這件事要查過才准寫。** 查的是 `pipe_server.cc` 的 HELLO 分支:關卡是 `h.proto < kMinProtocolVersion || h.proto > kProtocolVersion`,而回覆是**協商出來的** `ok.proto = h.proto`。所以 3→4 之後宣告 2 或 3 的舊 DLL 照樣被接受,只是永遠不送新訊息;反方向(新 DLL + 舊服務)靠既有的降版重試。⚠ **前提是新版本沒有動任何既有訊息的位元組佈局** —— v4 一個 Hello 欄位都沒加,所以 v3 的線路一個位元都沒變。加欄位的話這個結論就不成立。`


## 按鍵的等待沒有上限,而「連上又立刻斷」完全不受節流保護(分支 `winfix-usable`,2026-08-15,#93/#108)

⚠ **這一節不宣稱「間歇打不出中文」修好了。** 做的是三件可以驗的事:
把按鍵的等待**加上上限與作廢權**、讓重連的節流**涵蓋抖動**、
把記錄補到**下一次回報可以定案**。第一名(引擎佇列)排在第二名(設定檔 I/O)
前面的依據是機制推理,不是使用者機器上的量測 —— 而那正是第三件事存在的理由。

- `[2026-08-15] [winfix-usable/Windows → 全體] ⛔ **有上限的等待,如果沒有配「作廢權」,換到的缺陷比原本更糟。** 服務端處理一顆按鍵是 `queue_.Call(fn, 0)` = 永遠等,而 DLL 那側只有 150ms(`windows/common/key_deadline.h` 的 `kKeyTimeoutMs`;服務端那一側是 `kKeyDeadlineMs` = 100)—— 逾時的代價不是「打出英文」,是 `ipc_client.cc` 的 `Fail()` 第一句 `Close()` 把**整條連線丟掉**。所以要加上限。但只加上限的話:我們回 `handled=false`,DLL 照既有路徑把那個字元**自己補進宿主的文件**,而幾百毫秒之後那件遲到的工作又把同一顆鍵送進 librime —— **引擎組了字、宿主也打了字,兩邊分岔**,而且是靜默的。修法是 `WorkQueue::CallAbandonable()`:一個共享的 atomic,工作進來第一件事是 `exchange(true)`、呼叫端逾時之後也去 `exchange(true)`,所以「本體跑了」與「呼叫端放棄了」**剛好發生一件**。⚠ **誠實標明殘留窗口**:呼叫端搶不到的那一格(工作剛好在同一瞬間進到本體)關不掉,librime 沒有中途取消 —— 它被記成 `本體作廢=0` 寫進 service.log,不假裝沒有。⚠ **給四端的一句**:凡是把「無上限的等待」改成「有上限」的地方,先問「逾時之後那件工作還會不會動使用者看得到的狀態」;會的話,上限與作廢權必須**同一次**做完。`
- `[2026-08-15] [winfix-usable/Windows → 全體] ⛔ **重連的退避只涵蓋「連不上」,完全不涵蓋「連上又立刻斷」。** `LinkState::OnConnected()` 一被呼叫就把 `backoff_ms_` 歸零,於是「連上 → 第一顆鍵逾時 → 斷線 → 500ms 後再連上」這個迴圈**永遠停在 initial_backoff_ms**,一次都不會升級。而每一輪重連在服務端都是一次 SESSION_NEW(實測 `rs_session_create` 442~753ms)加一次寫回使用者詞典 —— **節流失效本身就在生產把下一顆鍵擠爆的慢工作**,這是一個正回饋迴圈。修法:歸零延到 `OnExchangeOk()`,而且要求這條連線活過 `stable_ms`(預設 2000,**這個值沒有量測依據,是推測**;行為由測試釘住,值可以再調)。⚠ 使用者感覺得到的那一半沒有變:`MayEatKey()` 只看 phase,連上就立刻能打字。⚠ **給四端的一句**:任何指數退避,列舉「什麼算恢復」時要問「連得上」與「撐得住」是不是同一件事;答案幾乎永遠是「不是」。`
- `[2026-08-15] [winfix-usable/Windows → 全體] ⛔ **`OnTestKeyDown` 說「吃」之後,`OnKeyDown` 的每一條出口都必須也吃 —— 而 `SendKey` 失敗那一條以前是 `return false`。** 宿主(以及 CUAS 相容層)在聽到「吃」的當下就放棄了自己的預設處理,事後改口那顆鍵回不去,它就這樣消失。使用者看到的不是「打出英文」,是「按了完全沒有反應」。⚠ 這個檔案自己在 `text_service.cc:1158-1167` 寫著「掉進兩邊中間的黑洞」,又在 1540 寫「唯一的例外是連線在兩趟之間斷掉,而那條路上面已經放行了」—— **後面那句是錯的**,而兩句相隔 380 行、互相矛盾地共存了好幾輪。⚠ **給四端的一句**:兩趟式的按鍵協議(Test / Key)只有一條不變式,而它是**全稱**的:第一趟說吃,第二趟的**每一條 return** 都要吃。程式碼裡出現「唯一的例外是…」時,那句話要有一個可執行的判準,不然它就只是一段散文。`
- `[2026-08-15] [winfix-usable/Windows → 全體] ⛔ **一顆按鍵在核心那一側只有「一份」預算,不是「一趟一份」。** #93 的第一輪只把 `ProcessKey` 收成有上限,而 `case Op::kKey` 那條路上**另外兩個出口**照舊是無上限的:`Ctrl+空白`/輕點 Shift → `Engine::ToggleAsciiMode()`,以及 `Ctrl+Shift+F` → `ReadBackStatus()` → `CurrentResult()`(**兩趟**)。四道新守門一條都沒紅,因為它們認的是 `Post("按鍵")` 這個字串與 `Engine::ProcessKey` 這個函式名 —— 而 commit 標題讓下一個人相信這件事已經關掉了。⚠ 補完之後還有第二件事:簡繁那條要走兩趟,兩趟各給一份完整預算的話最壞是 200ms,而 DLL 只有 150ms —— 「服務端必須先放棄」那條 static_assert 守的關係會在**執行期**被破壞,而它一個字都看不到。做法:呼叫端記下這顆鍵是什麼時候進來的(⚠ 要在讀設定檔**之前**),每一趟都問 `RemainingKeyBudgetMs()`;剩 0 就**不入列**(0 傳進 `queue_.Call` 的意思正好是「永遠等」)。⚠ **給四端的一句**:守門要從**呼叫點**把出口數出來,不要認函式名或字串 —— 「這條路上所有會進佇列的出口」與「我記得的那一個」是兩件事,而第二件會過期。`
- `[2026-08-15] [winfix-usable/Windows → 全體] ⛔ **「什麼都不做」包含不碰 UI —— 而「沒有逾時」不等於「這一份是現況」。** 簡繁快捷鍵在引擎回讀不到、或方案根本沒宣告字形開關時回 false = 什麼都沒做,但那一格照樣把一份**預設建構**的快照推進 UI:`items` 是空的 → 候選窗在使用者組字到一半時被收掉;旗標全 0 → `SnapshotFlagsAreUsable()` 仍然是 true(它只看「引擎沒準備好」那一個位元),於是狀態列把中/英寫成「中」、簡繁那格整個消失,而引擎可能正在英數模式、什麼都沒變。判準因此不能只問 `timed_out`,要問「**這一份 Result 是不是引擎跑完之後的現況**」——而它也不是 `handled`(引擎不吃這顆鍵時 handled=false,但快照是現況)。⚠ **給四端的一句**:凡是「拿不到結果就回一個預設物件」的路徑,先問那個預設物件會不會被畫到畫面上;全 0 在多數狀態模型裡都是一個**合法而且好看**的值,那正是它危險的地方。`
- `[2026-08-15] [winfix-usable/Windows → 全體] ⛔ **診斷記錄的節流要用「速率」,不要用「一輩子的總量」。** 服務端連線進出那兩行原本是「一輩子 64 條」,理由寫著「service.log 是 1 MiB 的環形檔,先被捲掉的正好是稀有的那一種」。**那個理由是錯的**:大小檢查一輩子只在**行程啟動時**做一次,跑的期間檔案只會長、沒有任何一行會被捲掉,超過上限時丟掉的是**整份**、而且發生在下一次啟動。於是那道額度擋掉的不是雜訊,**是最近的訊號** —— 13 個宿主 × 每個兩條連線,光登入就 26 條,額度在登入後幾十分鐘就沒了,而使用者下午出事去撈記錄時那組行早就不寫了。改法:令牌桶(桶滿 = 登入那一波看得到「連續好幾行」的樣式;之後每分鐘回補一條 = 八小時後還在寫),被壓掉的則數由下一行帶出去(那個數字本身就是重連迴圈最強的訊號),另外把「檔案無限長」交給**執行期**的大小檢查兜底。⚠ **給四端的一句**:節流的參數要拿「使用者出事的那個時刻」去驗,不是拿「這一行會不會太吵」去驗;而那一條驗得起來的前提是節流本身是純邏輯(時間由呼叫端傳進來)。`
- `[2026-08-15] [winfix-usable/Windows → 全體] ℹ️ **兩則更正,兩則都是這條線自己寫錯的。**(a) 「`engine_not_ready_` 沒有任何一條路清得回來」**是錯的** —— 那一格是 `exchange(算出來的值)`,下一份不帶該旗標的快照就會把它清成 false。那句話被抄進四個地方(含一則 commit message,改不了,所以記在這裡),而它自己的下一句「會一直說謊到下一顆成功的按鍵為止」正好推翻它。留著會讓下一個人去補一條不缺的清除路徑。(b) 「宿主 UI 執行緒最壞停住 ≤200ms」**低估了一半**:一顆鍵在 DLL 那側走**兩趟**管道(送鍵+等回覆的 Exchange、以及其後單向送 caret 位置),兩趟各拿一份完整預算;而 Exchange 內部本來更糟 —— 寫與讀各拿一份,寫入迴圈**每一塊**都重新給一次逾時,連絕對上限都沒有。修法:Exchange 內部改成共用一個**絕對 deadline**,再把「一顆鍵 = 幾趟」寫成具名常數讓斷言去守它。⚠ **給四端的一句**:「最壞會停多久」要**算**出來(單趟預算 × 趟數),不要用單趟的那個數字冒充;而註解裡的保證若沒有被某個算式頂著,它遲早會與程式碼分家。`
- `[2026-08-15] [winfix-usable/Windows → 全體] ℹ️ **記錄要補的是「額度」,不是「行數」。** tsf.log 的按鍵記錄只有 5 行的行程額度、連線失敗只有 6 行,而**唯一沒有額度的那一行**(`ipc_client.cc` 的「連線就緒」)正好是抖動時每 500ms 寫一次的那一行 —— 它會把 400 行的行程額度吃光,把 `ActivateEx` 那幾行真正有價值的訊息擠掉。改法:那一行前 3 次照寫,之後換成每 10 秒最多一行的**累計摘要**(重連幾次、上一條活了多久、斷線期間放行幾鍵、兩趟之間斷線時**補了幾個字元**、以及**吃掉而且什麼都沒做**幾鍵 —— ⚠ 後面兩個一定要分開數,理由見下面 2026-08-15 那一則);服務端則補 `KEY_MS=`(帶**設定讀取分項**與佇列的三個既有診斷欄位)與連線進出各一行。⚠ 設定讀取那一個分項是刻意的:沒有它,第一名(引擎佇列)與第二名(設定檔 I/O)在總時間上長得一模一樣。⚠ **給四端的一句**:診斷記錄的容量是**共享**的,所以「多加一行」永遠是在跟別的行搶位置;沒有額度的那一行不是最重要的那一行,它只是最吵的那一行。`
- `[2026-08-15] [winfix-usable/Windows] ℹ️ **順手修掉一句與程式碼不符的註解:「設定只在真的輕點了 Shift 的時候讀…不在每一顆按鍵的路上」。** `settings_->Load()` 以前是無條件執行的,排在 `DecideKeyAction` 之前,於是**每一顆按鍵**都在連線執行緒上做一次 `CreateFileW + ReadFile + Parse`,而且握著一把與 `Save` / `AppendNetLog` 共用的行程層級 mutex(`AppendNetLog` 是整份讀 + 寫暫存 + `FlushFileBuffers` + `MoveFileExW`)。改成先用純函式 `ClassifyHotkey()` 分類、只有輕點 Shift 那一格才讀 —— 行為一個位元都沒變(`DecideKeyAction` 只有那一格用得到那個布林),而「不必重開才生效」這個要求仍然成立。`


## 字集守門在 Windows 上是死的，而每一關都是綠的（分支 `winfix-usable`，2026-08-15，#109）

四端共用**同一個位元組**的字集守門 patch
（`core/data/schemas/luminakey_charset.custom.yaml`，md5 `e0843a17…`），
`collect_charset_guard.sh` 把它原封不動複製成 luna_pinyin /
bopomofo / t9_pinyin 三份 `.custom.yaml`，而 Windows 的安裝程式
確實把它們跟 `lua/*.lua` 一起裝進使用者機器。
只是 Windows 的 librime 沒有編 librime-lua。

**這一輪沒有把 lua 補上去**（那是 #27 / #109 的路 A）。
做的是把這件事**變成看得見的**：一道會擋下建置的閘，
加上一份有工單、有到期日、會被印出來的缺口清冊。

- `[2026-08-15] [winfix-usable/Windows → 全體] ⛔ **「找不到這個元件」在 librime 裡是 `continue`，不是錯誤。** `src/rime/engine.cc:310-315`：`auto c = T::Require(ticket.klass); if (!c) { LOG(ERROR) …; continue; }` —— 只記一行錯就把那個 filter 跳過。於是：安裝程式編得出來、簽章正確、安裝成功、服務起得來、候選也出得來，而**那一層過濾整層不存在**，使用者一個錯誤訊息都收不到。⚠ **給四端的一句**：凡是「把行為寫在資料檔裡、由引擎依名字去找實作」的設計，都有這個形狀：**打包進去的處方簽，與那一端的引擎真的有那個元件，是兩件事**，而沒有任何現成的關卡會發現它們分岔。`
- `[2026-08-15] [winfix-usable/Windows → 全體] ⛔ **閘要問的是二進位檔，不是原始碼。** 新增 `scripts/verify_schema_components.sh`：把資料目錄裡每一份 `*.schema.yaml` / `*.custom.yaml` 的元件處方抽出來（包括 `engine/filters/@before last` 這種 patch 鍵的八種形狀），一次餵給 `rime_console --has-component`，由它呼叫 `rime::Registry::instance().Find()` 回答。**不維護任何手寫清單** —— 手寫清單會在下一次有人改建置選項時安靜地過期，而過期的方向剛好是綠燈。⚠ **四端可用**：同一支腳本，各端指向自己的 console 二進位檔就好。macOS / Android 這一輪**還沒有**接上（見下一則）。`
- `[2026-08-15] [winfix-usable/Windows → 全體] ℹ️ **已知缺口的放行形狀：不是一個開關，是一份有工單、有到期日、會被印出來的清冊。** 這道閘一加，Windows 現在就會紅（我們確實在打包 lua_filter），而這一版必須發得出去。新增 `scripts/lib/component_gaps.tsv`，格式**逐欄照抄** `scripts/lib/known_gaps.tsv`（本檔 2026-08-13 那一則的要求）；不共用**同一個檔案**的理由是 `release_check.sh` 第 4b 關要求 known_gaps.tsv 的每一條那一輪都被用到，加一行進去等於讓 Android 每次 push 都紅。四條規矩一起成立：沒登記→紅、過期→紅、**登記了而這一輪根本不缺→紅**（缺口補上的當下豁免自動變紅逼人刪掉）、以及 `--announce-in`：被放行的工單號必須**逐字**出現在 `windows/service/main.cc`，那一行會印進使用者的 service.log。**放行可以，靜默不行。**`
- `[2026-08-15] [winfix-usable/Windows] ℹ️ **量過了：這一輪 Windows 的方案只缺 `lua_filter` 一個元件。** 把 `core/data` 底下 13 份方案檔抽出來的 21 個不重複元件，逐個對 `gears_module.cc` / `core` / `dict` / `levers` 的註冊清單，只有 `lua_filter` 不在裡面（`fluency_editor`、`punctuator`、`recognizer`…全部都在）。所以這道閘上線不會突然多紅一堆不相干的東西 —— 規格担心的 `t9_pinyin`（本專案自撰）也一個都不缺。`
- `[2026-08-15] [winfix-usable/Windows → macOS / Android] ⚠ **閘現在只掛在 Windows 的打包路徑上，而它擋得住 Windows、擋不住「四端不一致」本身。** `scripts/build_native.sh` 的 `verify_lua()` 之後、`apple/scripts/build_macos.sh:346` 之後各接一次同一支腳本，才是這件事的完整形式。沒做的理由是 Android 那一支 `rime_console` 是 **Android ABI** 的執行檔（`third_party/build/rime_console.x86_64`），要在模擬器上跑才問得到答案 —— 那是一個有成本的決定，不應該在這一輪順手做。`
- `[2026-08-15] [winfix-usable/Windows → #27 / #109] ⚠ **路 A(Windows 也編 librime-lua)這一輪刻意沒做,三個障礙寫在這裡,不要下一輪再查一次。** 甲、**`windows/build.sh` 根本沒有「取一份釘住 commit 的第三方原始碼」這個機制**:`fetch_pinned` 在 `apple/scripts/build_macos.sh` 與 `scripts/build_native.sh` 各有 3 處,在 `windows/build.sh` 是 **0 處** —— 它只有 `fetch_librime()`,而那一支是 `scripts/fetch_librime.sh glog yaml-cpp leveldb marisa-trie opencc`,清單裡沒有 librime-lua。所以路 A 不是「掛個 symlink」,是要先補一條取原始碼的路。乙、**掛載方式**:Android / macOS 都用 `ln -s`,而 MSYS 的 symlink 行為取決於 winsymlinks 設定與開發者模式,Git Bash 上不可靠;要改用 `cmd //c mklink /J` 或 `cp -r`,並在建立後斷言 `plugins/lua/CMakeLists.txt` 讀得到。丙、**`LUA_BUILD_AS_DLL` 覆寫不掉**:`third_party/librime-lua/CMakeLists.txt:22-24` 在 `if(WIN32)` 底下**無條件**下 `set_property(SOURCE ${LUA_SRC} PROPERTY COMPILE_DEFINITIONS LUA_BUILD_AS_DLL)` —— 那是 SOURCE 屬性、寫在外掛自己的 CMakeLists 裡,沒有任何 cache 變數擋得住它,要改只能再加一份 `patches/librime-lua@*.patch`。它會不會真的出問題(從靜態庫 dllexport 在 MSVC 不是錯誤,只是符號會帶進最終連結)**沒有人在 MSVC 上實編過**,而這台 Ubuntu 證不了。`
- `[2026-08-15] [winfix-usable/Windows → 全體] ⚠ **不要把路 A 排進「修使用者現在的痛」那一版。** librime 缺元件是 `continue`(engine.cc:314),所以字集守門是死的**不會**造成「打不出中文」;反過來,把 lua 補上去會**新增一條候選歸零的路徑**(lua 端出錯 → LuaTranslation 一取就錯 → 整段候選被吃掉),那條的保底是 `patches/librime-lua@filter-passthrough.patch`,而同一個守門在 macOS 上就因為候選變成 0 被從 main 撤回過一次(#97)。在「間歇打不出中文」還沒定案的同一版把它開起來,是把兩件事攪在一起。⚠ 沙盒那一格反而不是障礙:`scripts/verify_lua_sandbox.sh` 是**純 Lua** 的探針(自己編一份 Lua 5.4.8 把兩層沙盒裝起來再真的去 os.execute),與平台無關;Windows 這一側 `build.sh` 的 `check_lua_sandbox()` 已經在守「patch 沒套上就 die」,是 fail-closed 的。`


## 副作用排在最後一趟;以及「工作跑完了」不等於「答案是現況」(分支 `winfix-usable`,2026-08-15,#93/#108,收尾)

這一節只有三則,兩則是**這條線自己造出來的**,第三則是查證後**刻意不修**的。
...
- `[2026-08-16 更正] [drop-desktop → 全體] ⚠ **上面那一則的第一句「產物一個位元組都沒有動」是假的 —— 實測推翻。** 把 `5fa5baa` 與 `916a22b` 兩版 `scripts/collect_data.sh` 的第 7 節各跑一次再比對:`default.custom.yaml` **逐位元組相同**,`default.custom.mobile.yaml` **不同**。差異是 `scripts/collect_data.sh:224-225` 那兩行 —— 它們寫在 `MOBILE_ONLY` 的 heredoc **裡面**,而 heredoc 裡的 `#` 是 YAML 註解、是產物的一部分(其中一行還頂到第 0 欄)。語意沒有變(`yaml.safe_load` 兩邊都解出 `schema_list` 4 項 + `key_binder/bindings`),**但那一則接下去推出的結論也跟著垮了**:「因此 `verify_core_data_fanout.sh` 判定沒有動到 core/data,這條分支不需要接進 `build.yml` 的 `on: push: branches:`」—— 那個判定本身就是這一輪查出來的**假綠燈**,成因是 `gen_data_changed()` 的 `normalize()` 把「開頭是 `#` 的行」一律當成不會被執行的註解丟掉,而在 heredoc 裡那不成立。⚠ 兩邊都修了:`collect_data.sh` 把歷史說明搬出 heredoc(產物只留一行對齊的註解),`verify_core_data_fanout.sh` 的 `normalize()` 改成對 heredoc 本文原樣保留,並補了兩個方向的自我測試(heredoc 裡面改註解算動到、heredoc 外面加註解不算)。⚠ **修完之後這條分支上那一關會紅**,而那是對的:產物確實變了,`drop-desktop` 確實不在 `build.yml` 的 `branches:` 裡;併回 `main` 之後它會綠(`main` 在每一份 `branches:` 清單裡)。`
...
## 桌面兩端收掉的覆核:三道跟著一起沒了的守門(分支 `drop-desktop`,2026-08-16)

- `[2026-08-16] [覆核 → 全體] ⛔ **上面那一則「四條裡三條有等價物」的 ✅ 少算了兩條,而那兩條當時 Android 一條都沒有。** 盤點時只比對了「Android 有沒有名字像的測試」,`ThemeParserTest` 裡確實有 counterpart 與 syllables 的字樣 —— 但前者(`:47`、`:62`)只寫死 default-light / default-dark **一對**,後者根本不存在。**盤點守門要看它的分母,不是看它叫什麼名字。** 兩條都已補上,見下兩則;`docs/theme-format.md` §10 那張盤點表已改成六列(五 ✅ 一 ❌),並把漏記的原因留在表上。`

- `[2026-08-16] [覆核 → Android] ✅ **補回全主題的 counterpart 掃描:`ThemeParserTest.everyThemeCounterpartResolvesToTheOppositeAppearance`。** 對 `core/themes/` **每一份**(分母是掃目錄的 `RepoFixtures.themeIds`,不是白名單)斷言 counterpart 載得起來、且 appearance 相反。⚠ **這條在 Android 是執行期真的在走的路**:`LayoutHost.kt:421` `current.counterpart?.let { loadTheme(it) }`、`RimeInputMethodService.kt:628`(§8.2 執行期規則第 2 條)。而 §8.2 第 3 條規定「counterpart 載入失敗 → 沿用當前主題,**不是**致命錯誤」(`LayoutHost.kt:403`)—— 失敗是**靜默**的。既有關卡一條都接不住:`counterpart` 在 `ThemeParser.kt:284` 只是一個字串欄位,**指不到東西不會產生 diagnostic**,所以 `allShippedThemesParseWithoutDiagnostics` 只數 `diagnostics.size` 是數不到的。植入驗證(兩個分支各跑一次,不是只看回傳碼):`sakura-dark` 的 counterpart 改成 `sakura-ligth` → `[ERROR] fatal.document_not_found(sakura-ligth)`,紅;改成 `default-dark`(深淺同向)→「appearance 相同(都是 DARK)」,紅。`

- `[2026-08-16] [覆核 → Android] ✅ **補回 `candidates.syllables` 四個欄位的綁定對帳:`ThemeParserTest.documentedSyllableFieldsAreAllBound`。** 欄位名**從 `docs/theme-format.md` §8.6.6.3 的欄位表當場讀出來**(不抄常數,抄的那一份會腐爛),與 `ThemeParser.SYLLABLES_KEYS` **雙向**對帳,並保留「`orientation` 是推導的、不是欄位」那條反向斷言。為此 `ThemeParser` 的 `CANDIDATES_KEYS` / `SYLLABLES_KEYS` 由 `private` 改成 `internal`(單元測試同模組看得到,`DiagnosticStrings.TERM_IDS` 已有先例)。⚠ **消歧欄現在只剩 Android 一個實作者**(`theme-format.md:3194` 寫「桌面端不渲染」),而隨附的十二份主題**只有 `intl-ios-light.yaml:128-129` 寫了 `placement`**,`trigger` / `max_items` / `height` 一份都沒寫 —— 所以它接不到既有的「零診斷」那一關上。植入驗證:把 `max_items` 從 `SYLLABLES_KEYS` 拿掉,跑**全樹** 871 項,**只有這一項紅**(`expected:<[]> but was:<[max_items]>`)—— 這正是覆核說的「拿掉了全樹沒有一關會紅」,現在有一關會紅了。`

- `[2026-08-16] [覆核 → 全體] ⚠ **`docs/settings-model.md` §5 的 ✅ 徽章與現在式宣稱已改對 —— 規範內容一個字沒刪。** `:527-533` 與 `:686-691`(§5.4)原本仍以現在式說「`apple/scripts/verify_user_dict.sh` 每次 CI 都跑,而且是**硬失敗**」,並要求「其他端實作時請照抄這個形狀」。那支腳本已隨 `apple/` 刪除,跑它的 `macos.yml` 也刪了;而 `custom_phrase` 的掛載**只有 macOS 端實作過**(實測 `grep -rln custom_phrase android/ scripts/ core/`,Android 側只命中 SchemaPreflight 與方案 yaml)—— 這一節現在**零實作、零守門**。照本檔 §4 已經用過的形狀(`> ⚠ 2026-08-16 起,這條規範實際上沒有機械檢查在跑`)改寫:徽章縮回「**內容**驗過了(2026-08-09,真 librime)」,把「誰在守」單獨標成 ⚠,§5.4 標題加註「這個形狀今天沒有任何一端在跑」並寫明撿回來的指令(`git checkout desktop-final-5fa5baa -- apple/scripts/verify_user_dict.sh`)與缺的那一段(那支腳本是 macOS 專屬、直接在本機 exec `rime_console`,Android 的 `run_console_test.sh` 編的是 Android ABI、要 adb push)。**失敗情境**(這就是為什麼不能只留 ✅):下一個接手使用者詞庫的人讀到「每次 CI 都跑、硬失敗」,會認定 TSF 編碼格式受機器保護,照 §5.3 把**空白分隔**的編碼寫進 `custom_phrase.txt` —— 那正是 2026-08-09 量到「打不出任何字」的那個錯。⛔ **今天仍然沒有人在守這一節**,改的只是那句「有人在守」。`

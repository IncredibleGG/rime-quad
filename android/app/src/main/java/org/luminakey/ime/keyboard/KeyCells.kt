 ...
 *      ⚠ 使用者實際看到的**不是**「縫被擠沒了的鍵盤」——量出來是相反的：
 *      內縮（縫）是活下來的那一個，**鍵才是消失的那一個**。screenW ≲ 32 dp
 *      起窄鍵的可見寬度就是 0，≲ 20 dp 起整列都看不見。**但命中格仍然在、
 *      仍然打得出字**——這正是這條規矩要保住的東西：不崩、按得到。
 *      ⚠ 不選「等分」：等分會讓空白鍵縮成跟 `a` 一樣寬，而這種狀態多半是
 *        暫態（轉向、視窗重建），閃一下重排過的鍵盤比擠一下更難看。
 *        ⚠ 但要誠實說：寬度趨近 0 時這條規矩**本來就收斂到等分**，
 *          所以這個理由只在「還畫得出東西」的那一段成立。

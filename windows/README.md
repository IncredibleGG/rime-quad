...
* 首次部署(整理字詞;實測值與等待預算見 `windows/common/first_run_timing.h`,
  數字只有那裡有一份)提早開始,而不是等到他第一次按鍵;
...
⚠ **2026-08-15 起 U1 / U2 拿得到一張圖了(但那不等於驗過)。**
`windows.yml` 的 `install-x64` 會在 runner 上把設定視窗開出來,五頁 ×
深淺兩份各拍一張,artifact 叫 **`windows-settings-screenshots-<sha>`**,
檔名是 `settings-p<頁>-<light|dark>.bmp`。同一個 job 的下一步
(`windows/check_ui_shots.sh`)會在**少一張、大小是 0、整張同一個顏色、
或深淺兩張位元組相同**的時候讓 job 變紅 —— 一張全黑的圖比沒有圖更糟,
它在 artifact 上看起來像有交付。

那幾張圖**證明不了**的東西仍然留在下面這張表裡:字體(runner 多半沒有
Segoe UI Variable / Microsoft JhengHei UI,走的是退化路徑)、真實 accent
(runner 是預設藍)、125%/150%/200% 的 DPI(runner 是 96)、
滑過與按下(runner 上沒有人在動滑鼠)、以及「好不好看」。

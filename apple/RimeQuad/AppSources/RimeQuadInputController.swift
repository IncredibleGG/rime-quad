...
final class RimeQuadInputController: IMKInputController {
    ...
    // ───────────────────────── 輸入模式 ─────────────────────────
    //
    // ⚠ 這一段修的是真機回報的缺陷:使用者選了 `…RimeQuad.Hans`(簡體),
    //   打 `hao le` 卻得到「號」這個繁體字 —— 輸入模式與方案沒有綁在一起。
    //   判定規則是純邏輯,在 RimeQuadKit/InputModeBinding.swift,有單元測試。
    ...
    override func menu() -> NSMenu! {
        ...
        let about = NSMenuItem(title: T("關於 RimeQuad", "关于 RimeQuad", "About RimeQuad")[lang],
...
enum SettingsLauncher {
    static var settingsAppURL: URL? {
        ...
        let url = res.appendingPathComponent("RimeQuadSettings.app")
        return FileManager.default.fileExists(atPath: url.path) ? url : nil
    }

    static func open(arguments: [String]) {
        guard let url = settingsAppURL else {
            NSLog("RimeQuad: 找不到 RimeQuadSettings.app —— 這份 .app 的內容不完整")
            return
        }
        ...
        NSWorkspace.shared.openApplication(at: url, configuration: cfg) { _, error in
            if let error {
                NSLog("RimeQuad: 打不開設定視窗:\(error.localizedDescription)")
            }
        }

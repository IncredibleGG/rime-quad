...
//   · Shift 按住時使用者用**滑鼠**(或觸控 / 手寫筆)動了宿主的文件
//     —— 呼叫端要呼叫 OnOtherInput(),見下面那一整節
//
// ══ ⚠ 看不見的那一種輸入:滑鼠 ═════════════════════════════════════
//
// 上面那句「中間什麼都沒發生」,這個狀態機只驗得到**按鍵**那一半。
// 滑鼠一顆按鍵事件都不會產生,所以在它眼裡
//
//     按住 Shift → 滑鼠點一下 → 放開 Shift
//
// 與一次乾淨的輕點**逐位元相同**。而那一串正是「延伸選取」的標準手勢:
// 任何人在文件裡選一段字都會做,而這顆鍵預設是開的。
// 這一條是實跑量到的(見 docs/coordination.md 的 `[2026-08-12] [winbar]`
// 第三則),不是讀碼推的。
//
// ⚠ **不要在這裡想辦法看到滑鼠。** 這一層是純邏輯,而且平台那一層
//   也看不到 —— TSF 的四支 sink 只收按鍵。唯一乾淨的通道是「宿主說
//   這份文件的選取被動過了」(`ITfTextEditSink::OnEndEdit` 的
//   `GetSelectionStatus()`),而那件事**呼叫端才問得到**。所以這裡
//   只留一個入口 OnOtherInput(),由呼叫端在收到那個通知時呼叫。
//   為什麼不用 `GetAsyncKeyState` 之類的東西去問滑鼠:那條紅線見
//   `common/hotkey_policy.h` 與 `windows/audit_offline_win.sh`。
//
// ⚠ 判準用「選取變了」而不是「有沒有人點下去」是**刻意**的,而且它
//   剛好比看到滑鼠更準:一次沒有把游標挪動的點擊,對這個判準來說
//   本來就等於什麼都沒發生。反過來,觸控與手寫筆挪動游標時走的是
//   同一個通知,不必各認一次。
//
// ⚠ 涵蓋不到的那一格:宿主**不回報**選取變化時(它得呼叫
//   `ITfContextOwnerServices::OnSelectionChange`,而那是宿主的義務,
//   不是我們能保證的),這條路上什麼都不會來,S4 在那個宿主裡仍然會
//   誤切一次。那一格只有真機驗得到,已列進 #48。
//
...
class ShiftTapState {
 public:
  ...
  // 「有一種我們看不到的輸入,剛剛動了宿主的文件。」
  //
  // 呼叫端**必須**在收到宿主的選取變化通知時呼叫(TSF 那一側是
  // `ITfTextEditSink::OnEndEdit` 且 `GetSelectionStatus()` 為真),
  // 以及宿主自己把組字結束掉時(`OnCompositionTerminated`)。
  // 少了它,「按住 Shift → 滑鼠點擊 → 放開 Shift」——**延伸選取的標準
  // 手勢** —— 會切一次中英。理由與涵蓋範圍見檔頭「看不見的那一種輸入」。
  //
  // ⚠ 它是**作廢**(kArmed → kPoisoned),**不是** Reset()。
  //   Reset 會回到 kIdle,於是「按著 Shift 不放(自動重複中)→ 點一下
  //   → 繼續按著 → 放開」裡,點擊之後的下一個 down 會**重新開始**,
  //   然後那個 up 就切了 —— 與 kPoisoned 存在的理由是同一條。
  //
  // ⚠ kIdle 與 kPoisoned 時它什麼都不做。這一點是刻意的,而且是這個
  //   入口可以隨便接的原因:我們自己動文件(上屏、組字)也會讓宿主發出
  //   同一則通知,而那些時刻狀態機一定不在 kArmed —— 動文件永遠是某顆
  //   非 Shift 的鍵引起的,那顆鍵早就先把這一段毒掉了。所以呼叫端不必
  //   分辨「這次是不是我們自己弄的」,也就不必為此多存一個位元。
  void OnOtherInput();

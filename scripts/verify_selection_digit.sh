...
#              (非空、且不等於組字中的那一串)。
#          3b. **換一個順序再問一次同一件事。** 上面三步共用同一個開場,而第 1 步
#              還先點過一次高亮那一格、真的上屏了一個詞 —— 那只是**一種**順序。
#              工單 #99 的判準是「現在有沒有在組字」,而那個判準在不同順序之下
#              有可能給出不同答案;序號對使用者的承諾裡沒有「先怎樣再怎樣」
#              這個但書,所以這一步走另一條路:
#              **先按數字(沒在組字)→ 再組字 → 再按數字**。
#              結果併進同一個 `yes`:任何一種順序按不到,這一格就不是 yes。
#
#              ⚠ **2026-08-14 訂正:它現在是一條被證實的回歸守門了,而在此之前
#                它是空包彈。** 兩件事同時翻面,而且是同一個原因:
#
#                從前的紀錄是「前半段沒有東西送出去:沒在組字時按 `1` / `3` /
#                `9` 什麼都不會發生,而 `2` / `4` / `5` 會正常打出數字」,並且
#                把它當成另一個(已經寫進 `docs/coordination.md` 的)毛病放過。
#                **那不是另一個毛病,那就是這一支在守的東西的指紋。**
#                `Modifier.pointerInput(key)` 的手勢協程在一顆鍵**第一次被碰到**
#                時才啟動,而且此後不再重啟 —— 它捕捉到的「按下去要做什麼」
#                從此凍住。第 2 / 3 步已經在組字中碰過 `1` 與 `3`(當時的答案是
#                「選第 N 個」),所以 3b 的第一下拿的是那份過期答案,而當下
#                一個候選都沒有 —— 什麼都不會發生。`2` / `4` / `5` 之所以正常,
#                只是因為它們在那一輪之前沒有被碰過。
#
#                同一個凍住反過來就是破壞性的那一半:一顆在**沒組字**時被碰過的
#                數字鍵,之後在組字中會照樣把數字送進引擎 ——
#                `cn-t9-pinyin-numrow` 上那就是 `33⋯`(組字被毀)。
#                修法見 `KeyboardView.kt` 的 `keyFire`。
#
#                所以底下 3b 的第一下多了一條**前提斷言**:那一下必須真的
#                做了什麼。它在修好之前四列全紅,修好之後四列全綠。
#
...
PYK
  ...
  # ── 3b. 先按數字(沒在組字)→ 再組字 → 再按數字 ──────────────────
  # 見檔頭第 3b 步。**這一格與上面三步問的是同一個承諾**,只是換一條路走過去;
  # 所以它不是新的守門,而是同一格答案的第四個必要條件。
  ORDER2=no
  open_target; read_frame || { fail "$LAYOUT:第 3b 輪讀不到 frame"; continue; }
  tap_key "$K3" || { fail "$LAYOUT:第 3b 輪(沒在組字)點不到送 3 的鍵($K3)"; continue; }
  sleep 1.2
  B_IDLE_COMMITTED="$(committed_len)"
  B_IDLE_CLEN="$(composing_len)"
  info "3b:沒在組字時按 3 → 已上屏 $B_IDLE_COMMITTED 字、組字區 $B_IDLE_CLEN 字"
  # ⛔ **前提:閒置那一下必須真的做了什麼。**
  #    3b 存在的理由是「換一條順序走到同一個承諾」,而它的前半段是
  #    「沒在組字時按 3」。那一下若**什麼都沒發生** —— 沒有東西上屏、也沒有
  #    開始組字 —— 這條順序根本沒走過,後面那個 `ORDER2=yes` 是白給的。
  #    2026-08-14 在修好之前實測,四列全紅,而且 0/0 正是缺陷的指紋:
  #    手勢協程在這顆鍵**第一次被碰到**時把「按下去要做什麼」凍住,第 3 步
  #    已經在組字中碰過 K3(當時的答案是「選第 3 個」),於是 3b 的第一下
  #    拿的是那份過期答案,而當下一個候選都沒有 —— 什麼都不會發生。
  #    (檔頭從前把它記成「按了沒反應」,那不是另一件事,那就是這件事。)
  #
  # ⚠ 判準是「有沒有動靜」,**不是**「有沒有送出數字」:`bopomofo-dachen`
  #   的 `3` 是聲調鍵 ˇ,閒置按下去被 speller 收走、開始組字(實測
  #   已上屏 0 字、組字區 1 字)—— 那一下**確實走到了**,那一列是 `no`
  #   的理由在第 3 步,不在這裡。要求它送出數字等於在這裡先判它是 yes。
  #
  # ⚠ 它守不到的:在有整排數字鍵的佈局上,閒置按 3 若改成「開始組字」而不是
  #   「打一個 3」,這一條照樣綠。那是另一種缺陷,今天沒有人量到過。
  [ "${B_IDLE_COMMITTED:-0}" -gt 0 ] || [ "${B_IDLE_CLEN:-0}" -gt 0 ] || \
    fail "$LAYOUT:3b 的前半段什麼都沒發生(已上屏 $B_IDLE_COMMITTED 字、組字區 $B_IDLE_CLEN 字)—— 這條順序等於沒測到"
  type_compose || { fail "$LAYOUT:第 3b 輪打不進去"; continue; }
  B_PRE="$(committed_len)"
  B_CLEN="$(composing_len)"
  tap_key "$K3" || { fail "$LAYOUT:第 3b 輪(組字中)點不到送 3 的鍵($K3)"; continue; }
  sleep 1.2
  B_POST="$(committed_len)"
  B_CLEN_AFTER="$(composing_len)"
  info "3b:組字之後再按 3 → 已上屏 $B_PRE → $B_POST 字、組字區 $B_CLEN → $B_CLEN_AFTER 字"
  info "     按完的鏡射:$(mirror_state)"
  # 判準與第 3 步逐字相同:有沒有東西**真的上屏**(len − 組字區)。
  [ "${B_POST:-0}" -gt "${B_PRE:-0}" ] && ORDER2=yes
  info "3b(先按數字再組字)的答案:$ORDER2"

  # ⛔ 四件事都要成立才算 `yes`(fail-closed):
  ...
  #        `MGGAM3`)而輸入框仍然是空的;
  #    (d) **換一個順序(3b)也要成立。** 序號對使用者的承諾裡沒有「先怎樣再怎樣」
  #        這個但書 —— 某些順序按不到,就是「畫得出來、按不到」。
  ENGINE=no
  if [ "${T1LEN:-0}" -gt 0 ] && [ -n "$T1" ] && [ "$D1" = "$T1" ] \
     && [ "${C3_POST:-0}" -gt "${C3_PRE:-0}" ] && [ "$ORDER2" = yes ]; then
    ENGINE=yes
  fi
  info "引擎側的答案:$ENGINE"
  # ⛔ 兩種順序答案不同 —— 把它單獨叫出來。併進 ENGINE 之後表也會對不上而紅,
  #    但那句紅字說的是「表寫 yes、實測 no」,看的人查不出**是哪一條路壞了**。
  if [ "${C3_POST:-0}" -gt "${C3_PRE:-0}" ] && [ "$ORDER2" != yes ]; then
    fail "$LAYOUT × $SCHEMA:**同一顆數字鍵,操作順序不同結果不同**。" \
         "守門原本的順序(先點高亮那一格上屏一次)按 3 選得到;而「先按數字、" \
         "再組字、再按數字」按 3 選不到(已上屏 $B_PRE → $B_POST 字、" \
         "組字區 $B_CLEN → $B_CLEN_AFTER 字)。" \
         "使用者按下去毀掉的是自己剛打好的組字。"
  fi

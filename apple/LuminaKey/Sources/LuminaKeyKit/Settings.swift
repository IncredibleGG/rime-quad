...
public struct LuminaKeySettings: Equatable, Sendable {

    /// 目前的結構版本。**遷移的唯一依據**,不要拿欄位存不存在來猜。
    public static let currentVersion = 1

    public var version: Int = LuminaKeySettings.currentVersion
...
public enum SettingsCodec {
    ...
    public static func decode(_ data: Data) -> LuminaKeySettings {
        guard let obj = try? JSONSerialization.jsonObject(with: data),
              let dict = obj as? [String: Any] else { return LuminaKeySettings() }
        return decode(dict)
    }

    public static func decode(_ d: [String: Any]) -> LuminaKeySettings {
        var s = LuminaKeySettings()
        s.version = int(d["version"]) ?? LuminaKeySettings.currentVersion
    ...
    public static func encode(_ s: LuminaKeySettings) -> Data {
        var d: [String: Any] = [
            "version": LuminaKeySettings.currentVersion,
...
public enum SettingsMigration {
    ...
    /// (`~/Library/Application Support/LuminaKey/` 會被 iCloud/Dropbox 帶走),
    /// 舊版看到新版的檔案時,能讀的欄位照讀,但**不覆寫** ——
    /// 覆寫的話新機器上調好的東西會被舊機器洗掉,而且沒有任何跡象。
    /// 這個判斷交給 `SettingsStore.save` 的 `isDowngrade`。
    public static func migrate(_ input: LuminaKeySettings) -> LuminaKeySettings {
        var s = input
        if s.version < 1 {
    ...
    /// 讀進來的版本比本版新 → 不要寫回去。
    public static func isFromFutureVersion(_ s: LuminaKeySettings) -> Bool {
        s.version > LuminaKeySettings.currentVersion
    }
}
...
public final class SettingsStore {
    ...
    public static let changedNotification = "org.luminakey.settings.changed"

    private let url: URL
    private let io: SettingsFileIO
    private(set) public var current: LuminaKeySettings

    public init(url: URL, io: SettingsFileIO = DiskSettingsIO()) {
        self.url = url
        self.io = io
        self.current = io.read(url).map(SettingsCodec.decode) ?? LuminaKeySettings()
    }

    /// 從磁碟重讀。回傳是否真的變了 —— 沒變就不必叫 UI 重畫。
    @discardableResult
    public func reload() -> Bool {
        let fresh = io.read(url).map(SettingsCodec.decode) ?? LuminaKeySettings()
    ...
    /// 改一項並存檔。回傳是否真的寫了。
    @discardableResult
    public func update(_ mutate: (inout LuminaKeySettings) -> Void) -> Bool {
        var next = current
        mutate(&next)
        ...
        next.version = LuminaKeySettings.currentVersion
        current = next

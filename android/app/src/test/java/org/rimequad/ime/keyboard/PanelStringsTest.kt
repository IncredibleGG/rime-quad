 ...
 * App 首頁裝完就不會再開了。
 *
 * 漏掉一句的下場和 [org.rimequad.ime.StringCatalogTest] 守的一樣安靜:畫面
 * 不會壞、log 不會叫、build 不會紅,只是一個英文系統的使用者在自己的鍵盤上
 * 看到一句繁體中文。
 *
 * 這裡守兩件 [org.rimequad.ime.StringCatalogTest] 守不到的事。
 */
class PanelStringsTest {
    ...
    private fun kotlinSources(): List<File> =
        File("src/main/java/org/rimequad/ime/keyboard").listFiles().orEmpty()

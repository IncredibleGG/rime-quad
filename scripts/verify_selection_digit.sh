...
#
#              ⚠ **這一格加進來的時候是綠的,它不是一條被證實的回歸守門。**
#                加它的起因是覆核回報「換個順序 6/6 重現 `3⋯`」,而 2026-08-14
#                在 emulator-5558 / lumina_test2 上**量不出來**:這一條順序、
#                外加另外五種(全新開啟直接按 3、同一欄位連做兩次、按 1 之後
#                再組字按 3、只打兩鍵就按 3、按 9)全部正常上屏。留著它是因為
#                它真的多走一條路,不是因為它抓到過東西 —— 下一個人不要把
#                「它一直是綠的」讀成「它守住了什麼」。
#
#              ⚠ **而且前半段目前是空包彈:沒在組字時按 `1` / `3` / `9`
#                什麼都不會發生**(`2` / `4` / `5` 會正常打出數字;
#                `cn-t9-pinyin-numrow` × `t9_pinyin` 與 `cn-qwerty-numrow` ×
#                `luna_pinyin` 都量到)。也就是說對 `K3` 而言,3b 的第一下
#                等於沒按 —— 這條順序真正走到的差別只有「這一輪之前沒有
#                上屏過任何東西」。那個「按了沒反應」本身已經寫進
#                `docs/coordination.md`,它不是這一支在守的東西。
#

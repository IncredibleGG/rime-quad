...
- `[2026-08-10] [fix-gates→Android] ⚠ **`--plant bad-slot-ids` 對 `t9-pinyin` 是空包彈,它的第 2 關至今沒有被反向驗過。**
  接上 CI 的第一輪(run 31325521966,慢車道)就看見了:植入之後
  `cn-t9-pinyin` 與 `cn-t9-pinyin-numrow` 都紅了(`ERROR=宣告的格位 id 在這一層找不到`),
  **`t9-pinyin` 照樣 PASS**,讀得到 `ni mi` 與 `hao gan gao`。
  原因是 `core/layouts/t9-pinyin.yaml` **根本沒有 `syllable_slots`** ——
  那個 `sed` 一個字都改不到,而整支腳本仍然報「反向測試通過」。
  也就是說:它的消歧欄是**另一個機制**畫出來的(上方橫排的 chip,不是格內替換),
  而那條路徑沒有任何反向測試在守。
  已修的是**帳**不是機制:腳本現在逐份佈局判斷植入有沒有真的生效,
  「生效 N 份就必須紅 N 份」,而空包彈的那幾份會被**指名列出來**
  (「這一輪沒有反向驗到它們的第 2 關」)。一份都植不進去就直接判自己空轉。
  **機制那一半留給 Android 端**:要反向驗上方橫排那條路徑,需要另一種植入
  (例如把 chip 的資料來源指到不存在的東西),我沒有做 —— 那要動渲染端的知識。`

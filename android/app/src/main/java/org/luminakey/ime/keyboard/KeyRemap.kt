 ...
 *
 * 存的是**操作**而不是「改完之後的佈局快照」。理由與 [org.luminakey.ime.prefs.UserPrefs]
...
/** 每列寬度總和與 `units` 的容許誤差。與 [org.luminakey.ime.theme.LayoutParser] 一致。 */
 ...
 * 一顆鍵除了「在哪個格子」以外的全部內容。
 *
 * 用 `toString()` 序列化 [org.luminakey.ime.theme.Popup] 與 swipe：那些型別都是

...
//
//   · 服務端:Engine::ProcessKey 等引擎執行緒多久(kKeyDeadlineMs)
...
// 所以兩個數字一起放寬:
//
//   · DLL 端 150ms —— 這是宿主 UI 執行緒**最壞**會停住的時間,而且只在
//     引擎真的忙的時候發生。它換到的是那一顆鍵打出正確的中文。
//     相對地,50ms 換到的是一個錯的英文字母**加上**一次斷線,而斷線
//     之後那個宿主要重跑一次 SESSION_NEW(它自己的預算就是 300ms,
//     而且實測會到 442~753ms)—— 代價比多等的那 100ms 大得多。
//     150ms 也還在「按了有反應」這一側:人開始懷疑程式沒收到、於是
//     再按一次的門檻大約是 200ms。
//   · 服務端 100ms —— 150 扣掉餘裕。
//
...
// DLL 端每一顆按鍵的往返預算(tsf/ipc_client.cc)。
// 超過它 = 放棄 + 關掉連線,不是「這顆鍵慢了」。
constexpr int kKeyTimeoutMs = 150;

// 服務端等引擎執行緒的上限(service/engine.cc 的 CallAbandonable)。
...
// 低於它的按鍵不值得寫一行:1 MiB 的環形記錄檔會被自己的雜訊捲掉。
...
//     自稱「正在準備字詞」,那一格沒有任何一條路清得回來,而它正好
//     會誘導使用者去按「重新整理字詞」—— 一件真的要花幾十秒、而且會
//     再排一批慢工作進同一條 FIFO 的事。
//
// 同一支 pipe_server.cc 在「使用者把輕點 Shift 關掉了」那一格已經立過
// 同一條規矩(那裡的原話是:什麼都不做包含**不碰 UI**)。這裡是同一件
// 事,只是換一個理由走到同一格。
...
constexpr KeyUiAction DecideKeyUiAction(bool timed_out) {
  return timed_out ? KeyUiAction::kLeaveUiAlone : KeyUiAction::kUpdateUi;
}

...
enum SettingsPaths {
    ...
    static var workDir: URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("LuminaKeyStore")
    }
}
...
final class RemoteDeployer: Deployer {
    ...
    func deployAndWait(onTick: @escaping (Int) -> Void) -> DeployOutcome {
        ...
        // 對方可能還沒啟動(使用者還沒把 LuminaKey 選成輸入來源)。叫醒它。
        ...
        while true {
            ...
            case .noResponder:
                return .notStarted(reason:
                    "輸入法本體沒有回應。請先到「系統設定 › 鍵盤 › 輸入來源」把 LuminaKey 加進去並選用它,再試一次。")
            case .stalled:
...
final class SettingsModel {
    ...
    private let queue = DispatchQueue(label: "org.luminakey.settings.work")
    ...
    var settings: LuminaKeySettings { store.current }

    func edit(_ mutate: (inout LuminaKeySettings) -> Void) {
    ...
    ///   scripts/publish_apk.sh、scripts/downloads_server.py、.github/workflows/build.yml)
    ///   用的都是底下這一個。macOS 的方案市集因此**從來沒有抓到過索引**,
    ///   而症狀是一段「連不上」的紅字 —— 看起來像網路問題,不像打錯位址。
    ///   ⚠ 路徑那一段仍然是 `rime/…`,**不要**跟著產品改名一起改:
    ///   下載頁與應用內升級都指著它。
    static let defaultIndexURL = "https://pub-d6a54d2e5f5947e2b0b23fb8e27ce0a5.r2.dev/rime/schemas/index.json"
...
enum SettingsCommand {
    static let notification = "org.luminakey.settings.command"
...
enum SettingsAppMain {
    static func run() -> Never {
        // 設定介面通常是由輸入法本體叫起來的(那時已經搬過了),但它也可能被
...
enum SettingsMenu {
    static func build() -> NSMenu {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(withTitle: "About LuminaKey", action: nil, keyEquivalent: "")
        appMenu.addItem(.separator())
        appMenu.addItem(withTitle: "Hide LuminaKey Settings",
                        action: #selector(NSApplication.hide(_:)), keyEquivalent: "h")
        appMenu.addItem(withTitle: "Quit LuminaKey Settings",

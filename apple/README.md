...
- ⭐ **本輪新寫的每一處 UI 接線,拆掉都會紅。**
  上一輪四條缺陷的修法裡有五處接線落在 `AppSources/`,而 `swift test` 不編譯那個目錄,
...
- **使用者自己加的詞。** `verify_user_dict.sh` 一度證明:我們寫出的
  `custom_phrase.txt` 與掛載檔格式看起來對,但加了詞之後 librime 的候選裡
  **沒有那個詞**。「我的詞庫」那一頁因此刻意沒有上架。
  2026-08-09 找到原因並修好,那一關改成硬失敗,那一頁(現在叫「自己加的詞」)
  回到側欄。原因**不是**當時猜的任何一個:是編碼欄裡的空格 ——
  librime 拿使用者按出來的原始字串去查,要逐字元相等,`ni hao` 永遠查不到。
  那一關現在驗五件事,含**反向測試**(空白分隔的編碼必須查不到),
  而且對它自己植入過四個違規確認每一個都會紅在對的那一步。
  教訓:**改了之後還是紅,代表診斷錯了,不是改得不夠。**

### CI **驗不到**的(只有人在真 Mac 上跑才驗得到)

0. **「自己加的詞」那一頁,從加詞到真的打得出來。** CI 驗到的是
   **格式**(rime_console 直接餵給 librime),不是**那一頁**:
   「按了『加這個詞』有沒有寫進檔案」「按了『重新整理字詞』有沒有真的重新部署」
   「回到別的 app 打那幾個鍵會不會出現」這三段沒有任何自動化。
   → 驗收步驟:加一個詞(例如 黃小明 / huangxiaoming)→ 按「現在重新整理字詞」
   → 切到別的 app 打 `huangxiaoming` → 那個詞要排第一。
   再試一次**故意在編碼欄打空格**,應該當場被擋下來而不是安靜地存進去。
...
9. **修飾鍵。** 「輕點 Shift 切中英」靠的是 `flagsChanged` 的 release 事件。
   ...
   **但「真的按下去會不會切」仍然只有人驗得到**:
   ...
   ⌘C / ⌘V 有沒有被吃掉也還是沒驗過(這一輪多送了 flagsChanged,
   Command 鍵的 press/release 現在也會進 librime,更該試一次)。

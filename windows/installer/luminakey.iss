...
; ⚠ 完成頁那個 FinishedLabel 是一個**不會捲動**的靜態文字:放不下的部分
;   直接被切掉,而且切掉之後畫面上看不出少了東西。
;
;   走查證明現在那七段(Win+空白、開始功能表的設定、托盤圖示收在「^」裡、
;   簡繁在「文字」頁、首次整理字詞、診斷捷徑)在真機上是**看得完的** ——
;   他逐段講得出來。這一輪往上面加了三段(清單還沒出現要等一下、
;   它掛在哪一個語言底下、中英怎麼切),所以那塊區域要跟著變大。
;
;   WizardSizePercent 是 Inno 6 給這件事的正式旋鈕(只在 WizardStyle=modern
;   下有效,範圍 100–150)。130 的兩個方向各大 30%:高多三成、每一行也
;   多放三成的字,所以那塊區域**照原本的字寬算**約多七成。
;
;   算過的(照原本的欄寬、含段落之間的空行):舊的七段是 21 個顯示行,
;   而走查證明那 21 行在真機上看得完;新的十段是 30 個 —— 多 43%,
;   而空間多 69%。留餘裕是刻意的:下一個人多寫一句話不該立刻掉出畫面外。
;
; ⚠ 完成頁引用捷徑名字時用 {#ProductName} 而**不是** [name]:[name] 展開的是
;   AppName(= {#ProductNameZh},「…輸入法」),而 [Icons] 那兩行用的是
;   {#ProductName}。差一個詞的話,他照著完成頁去「開始」功能表搜尋會搜不到。
;   同一條理由,診斷捷徑的冒號在這裡也要用**全形 U+FF1A** —— [Icons] 那一行
;   用全形是因為半形會被 NTFS 當成交替資料流(見那一段的註解),
;   而完成頁若寫半形,他複製貼上去搜尋就對不上。
;
; ⚠ **這一行沒有在 Windows 上看過。** Linux 上沒有 ISCC,而截圖那條線拍的
;   是設定視窗、不是安裝精靈。真機上要看的是:完成頁最後一段
;   (「打不出中文:…並用記事本打開。」)整句都在、沒有被切掉,
;   而且歡迎頁沒有因為變大而看起來空蕩蕩。
...
FinishedLabel=[name] 已安裝在您的電腦上。%n%n切換輸入法:按 Win + 空白鍵,選擇「[name]」,清單上只會有一格。%n剛裝好時清單裡可能還沒有它,實測約 {#ProfileVisibleSec} 秒後才出現。那是 Windows 在更新清單,不是裝失敗 —— 再按一次就有,不必重裝。%n%n它掛在哪一格:您原本有中文的話就掛在那底下。沒有的話 Windows 會替您加一個「中文(简体,中国)」,它掛在那裡 —— 那一格預設打簡體,要繁體到「文字」分頁改。%n%n中文 / 英文(句子中間要打英文單字時):點那一小橫的第一格、按 Ctrl + 空白鍵、或輕點一下 Shift。%n%n開啟設定:「開始」功能表 →「{#ProductName} 設定」;方案、簡繁、候選字大小都在那裡。%n也可以按工作列右下角的系統匣圖示 —— Windows 11 預設把新圖示收在「^」裡面,要先點開那個箭頭。%n%n簡體 / 繁體在設定的「文字」分頁切換,系統匣圖示按右鍵也切得到。%n%n第一次使用時輸入法會在背景整理字詞,大約 {#FirstDeployLowSec} 到 {#FirstDeployHighSec} 秒;那之前打出來的是英文,這是正常的。%n%n打不出中文:執行「開始」功能表裡的「{#ProductName} 診斷：輸入法為什麼不能用」,或設定「進階」分頁裡的同一顆按鈕 —— 它會把原因寫成報告並用記事本打開。

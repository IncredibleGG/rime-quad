...
if [ -z "${SVC_OLD_PID}" ]; then
  ...
  if [ -z "${P13_LPIPE}${P13_LHS}${P13_LSESS}" ]; then
    ...
    # ── ⚠ 紅字叫人去看的東西,這裡就要**印出來** ────────────────────
    #
    #   run 2026-08-15 那一趟的紅字寫著「要看的是服務端 [pipe] 那幾行」,
    #   而那幾行**從來沒有進過 CI log**:p13-svc-new.log 躺在 runner 的
    #   工作目錄裡,跑完就沒了。`grep -n "\[pipe\]" log4.txt` 唯一命中的
    #   是那句提示本身 —— 一個指向自己沒有印出來的資料的紅字,等於叫人
    #   去猜。而這一格分不出來的兩件事(逾時 vs. 部署中 fail-open)
    #   要修的地方完全不同。
    #
    #   所以在報失敗**之前**先把那幾行倒出來。額度 40 行:這是一份給人
    #   讀的報表,不是把整份 service.log 貼進來。
    echo "  --- 新服務的 [pipe] 那幾行(按鍵= / 逾時= / KEY_MS)---"
    (tr -d '\r' < "${WORK}/p13-svc-new.log" 2>/dev/null \
       | grep -aE '\[pipe\]' | tail -40 || true) | sed 's/^/    /'
    echo "  --- 瘦 DLL 記錄裡「引擎沒有回答」那幾行 ---"
    # ⚠ 這一行以前印不出來:它與「引擎不吃這顆字元鍵」共用同一份額度,
    #   而後者每天都會走到,第一階段打完 nihao1 就用光了(log4:1170)。
    #   現在它有自己的額度(tsf/text_service.h 的 not_answered_trace_budget_)。
    (tr -d '\r' < "${WORK}/p13-oldhost-trace.log" 2>/dev/null \
       | grep -a '引擎沒有回答這顆鍵' | tail -20 || true) | sed 's/^/    /'
    note_fail "線路那三句話都成立,但宿主仍以 ${OLD_RC} 結束 ——
     壞在按鍵那一段(session 開了,但按鍵沒有走成)。
     ⚠ 打出來的是**半串拼音半串中文**(例如「ni好」)的話,那是
       引擎沒有回答那幾顆鍵、而 DLL 把原始字母補進了文件 ——
       上面那兩段就是分辨「逾時」與「部署中 fail-open」的依據,
       兩者要修的地方完全不同。不要回頭去查管道名或版本協商。"

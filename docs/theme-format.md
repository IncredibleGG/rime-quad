...
41. **每一份隨附主題都排得下**（§8.6.4.2）。第 40 條是公式，這一條是**逐份**
    ...
    ⚠ 同一條也要驗**出口**：每一份主題都必須留著展開面板那條路
    （`scroll: expandable` ＋ `expand_button.show: true`）。關掉它就等於讓
    「本頁還有畫不出來的候選」那個情境退回翻頁 —— 而那正是第 44 條禁止的。
...
45. ⛔ **右端最多一顆，而且量測扣掉的寬度就是畫出來的那幾顆**（§8.6.6.4）。
    翻頁鍵與展開鍵解決的是同一個問題（「還有更多」），兩顆一起出現是同一份
    資訊的第二份。而且：**本頁還有畫不出來的候選時，不得提供「下一頁」**——
    ...
    第 1 頁又是最後一頁 → `PAGER` ＋ **0 dp**；第 2 頁且本頁看不完 →
